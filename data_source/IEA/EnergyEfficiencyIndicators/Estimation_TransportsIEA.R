...
#OBJET      - Script de traitement des données IEA (Efficicency Indicators) / World Bank pour l'estimation d'indicateurs du niveau d'activité, de l'efficacité énergétiques et de la demande d'énergie pour les différents modes et sources d'énergie
#OUTPUT     - 1 ffichier csv agrégeant les indicateurs estimés à l'échelle des 12 régions Imaclim-R world ; et les 3 fichiers csv individuels utilisés pour calibrer Imaclim-R monde
#INPUT (1)  - Fichier xlxs "Energy Efficiency Indicators" de l'IEA (ATTENTION, nécessaire de le convertir en xlsx, la version initiale en xlsm ne peut être importée)
...
#INPUT (3)  - Fichiers produits à partir de sources de données complémentaires (p. ex. yearbook pour la Chine, données d'aviation)

#Notes de version : 
#les commentaires n'ont pas été tous mis à jour pour tenir compte des dernières corrections et choix méthodo (tenir compte des deux roues, aviation, Chine)
#le fichier agrégé n'a finalement pas été utilisé, à part à titre informatif pour montrer la cohérence entre les variables
#nouvelle version partant des fichiers natifs disponibles sur le serveur, créée par Thomas le 18/02/2022
#après l'application de plusieurs techniques d'imputation, la base WB comporte encore des données manquantes (pour toute la période) pour British Virgin Islands, Cayman Islands, French Polynesia, Gibraltar, Korea, Dem. Peopleâ???Ts Rep., New Caledonia, Somalia, Syrian Arab Republic ; ces pays sont retirés de la base utilisée ensuite

#Reste à faire : 
## recorriger les données Chine (cf. ci-dessous) - OK
## vérifier qu'on a bien toutes les bonnes variables - OK
## clarifier les unités - PJ
## corriger les valeurs négatives air - OK
## vérifier les ordres de grandeurs - OK
## créer le fichier de sortie
## implémenter dans le code l'usage de ce nouveau fichier de sortie
## identifier et mettre en commentaire initial les problèmes possibles à la mise à jour
...
### 0 - Paramètres de l'exécution du code
Display_RegressionGraphs <- "no" #changer en "yes" pour afficher les graphiques de régression
WB_endyear<-2018 #dernière année de données à peu près complètes pour WB dans le jeu de données utilisé
#NB : il serait nécessaire de convertir en variable la constante qui fixe le nombre de valeurs par pays par variable, en cas de mise à jour des fichiers d'origine et de l'année de fin
ToDelete_IncomePC <- c("Indonesia") #on remarque que dans la base, les valeurs de revenus pc de l'Indonésie sont aberrantes (négatives notamment), on les supprimera puis elles seront estimées à partir du PIB
Year_Calibration <- 2014 #année à laquelle on veut les données dans la base agrégée à l'échelle régionale (la base "pays" comporte des données sur toutes les années)
Convert_EnergyUnit <- 0.0238845896627 #Facteur de conversion des unités énergétiques pour la base régionale. Ici on passe de PJ (unité de la base de l'IEA) à Mtoe (unité de la calibration transport dans Imaclim à la date de création du fichier)
LoadGTAP_forAirCorrection <- "no"

### 1 - Chargement des fichiers de données et prétaitements

## 1.1 - Chargement et prétraitement de la base de la banque mondiale et du fichier d'agrégation en régions Imaclim-R world à partir des codes pays ISO3
setwd("~/")
CountryRegion<-read.csv2("~/imaclim_ISO3_aggregates.csv",sep="|")
WB_DataBase<-read.csv2("~/WDIData.csv",sep=",") # l'usage de la fonction "check.names = FALSE" pose problème avec filter ensuite, tout comme le fait de renommer les années (X1960 => 1960), je ne renomme donc pas. #colnames(WB_DataBase)<-c("Country_Name","Country_Code","Indicator_Name","Indicator_Code",1960:2019)
...
# On sélectionne les variables suivantes, utilisées pour les régressions et autres opérations :
...
# IS.RRS.TOTL.KM    : Longueur de routes ferrées
# EN.POP.DNST       : Densité
...
# IS.RRS.PASG.KM    : Passagers transportés par train
# IS.AIR.PSGR       : Passagers transportés par air


WB_DataBase_copy<-WB_DataBase #On le fait en deux temps et on fait une copie de la base à ce stade car un peu long de charger la base
WB_DataBase_copy[which(WB_DataBase_copy$Country_Name %in% ToDelete_IncomePC&WB_DataBase_copy$Indicator_Code=="NY.ADJ.NNTY.PC.KD"),5:ncol(WB_DataBase_copy)] <- "" #Effacement de certaines données originales pour l'Indonésie (valeurs négatives)
...
WB_DataBase<-left_join(WB_DataBase_copy,CountryRegion, by=c("Country_Code"="ISO3")) %>%
  ...
  filter(Year %in% c(2000:WB_endyear)) %>% #On s'arrête à la dernière année à peu près complète
  #On s'assure que les variables sont numériques
  ...
  #On impute les données 2000-2006 de Nauru en amont différemment car le procédé qui suit générait des valeurs négatives ; ici on affecte aux années 2000-2006 la valeur 2007
  ...
  #On va commencer par imputer les données démograpiques par interpolation puis extrapolation (très peu de données manquantes)
  group_by(Country_Name) %>%
  mutate(Population_WB=na.approx(Population_WB,x=Year,na.rm=FALSE))  %>% #Pas utile avec les donnes utilisées en 2022, mais possiblement à l'avenir - première imputation par interpolation
  mutate(Population_WB=approxExtrap(x=Year[which(!is.na(Population_WB))],y=Population_WB[which(!is.na(Population_WB))],xout=c(Year))$y)  %>% #Imputation par extrapolation, pour 7 années pour l'Erythrée dans la base utilisée initialement
  #On crée les données per capita qui serviront aux imputations des revenus/capita
  mutate(GDP_ConstantPrice_pc=GDP_ConstantPrice/Population_WB) %>% 
  mutate(GDP_LCU_pc=GDP_LCU/Population_WB) %>% #Finalement non utilisée
  #On crée des variables pour classer les pays * variables en fonction du nombre de données manquantes, qui déterminera la méthode d'imputation
  mutate(check_NA_GDP=ifelse(sum(is.na(GDP_ConstantPrice))==19,0,ifelse(sum(is.na(GDP_ConstantPrice))==18,1,ifelse(sum(is.na(GDP_ConstantPrice))>=1,2,3))))  %>% #On diagnostique le nombre de NA ; 0 si que des NA ; 1 si une valeur existante ; 2 si au moins deux valeurs existantes (mais au moins une donnée manquante) ; 3 si aucune valeur manquante.
  ...
  #On applique les différentes méthodes d'imputation au PIB/capita (ainsi on exclut la composante démographique de la variation du PIB)
  mutate(GDP_ConstantPrice_pc_new=ifelse( #Ici on interpole le PIB/capita dans le cas où il existe suffisamment de valeurs, sur la période où il existe des données (en dehors, c'est traité ci-dessous par extrapolation)
    check_NA_GDP==2, na.approx(GDP_ConstantPrice_pc,x=Year,na.rm=FALSE), ifelse(
      check_NA_GDP==1, mean(GDP_ConstantPrice_pc, na.rm=TRUE), GDP_ConstantPrice_pc # Cas où on dispose d'une seule valeur : on utilise cette valeur pour les autres années
    )
  )) %>%
  mutate(GDP_ConstantPrice_pc_new=ifelse( # on extrapole le taux de croissance du PIB/capita pour les pays où suffisamment de données sont disponibles (avant la première valeur disponible, après la dernière)
...
#On impute les données d'income/capita, dont les données manquantes étaient bien plus nombreuses que le PIB (à prix constant), en commençant par créer un modèle de régression non paramétrique, qui servira de base pour les pays où aucune données n'était disponible
...
WB_DataBase_2 <- WB_DataBase %>%
  mutate(Income_pc_loess=predict(loess.income.pc,GDP_ConstantPrice_pc_new, se = FALSE)) %>% #On crée une nouvelle variable basée uniquement sur le modèle de régression
  mutate(Income_pc_new=ifelse( #Ici on interpole le revenu/capita pour les valeurs manquantes sur la période où il y a des données
    check_NA_INC==2, na.approx(Income_pc,x=GDP_ConstantPrice_pc_new,na.rm=FALSE), 
    Income_pc )
  ) %>%
  mutate(Income_pc_new=ifelse( # on extrapole à partir des variations du PIB/capita pour les pays où suffisamment de données sont disponibles (avant la première valeur disponible, après la dernière)
  ...
  mutate(Income_pc_new = ifelse(
    check_NA_INC<=1, predict(loess.income.pc,GDP_ConstantPrice_pc_new, se = FALSE), #On utilise le modèle quand il existe une valeur ou moins sur la période
    Income_pc_new)) %>%
  ungroup() %>%
  mutate(Income_pc_new=ifelse(Income_pc_new<=0,Income_pc_loess,Income_pc_new)) %>% #On corrige pour les données inférieures à 0 qui peuvent être liées aux extrapolations - dans ce cas on essaie le  modèle de régression ...
  mutate(Income_pc_new=ifelse(Income_pc_new<=0,GDP_ConstantPrice_pc_new*mean(Income_pc/GDP_ConstantPrice_pc,na.rm=TRUE),Income_pc_new)) %>% #... et pour ce qui reste, on utilise une autre approximation : une fraction constante du PIB (calcuée sur toutes la valeurs)
  mutate(Income_pc_new=ifelse(Country_Name=='Kuwait'&Year%in%c(2000:2008),GDP_ConstantPrice_pc_new*mean(Income_pc/GDP_ConstantPrice_pc,na.rm=TRUE),Income_pc_new)) %>% #On corrige également des extrapolations très optimistes pour deux pays
  ...
  # On impute les autres données : Rail_Length
  group_by(Country_Name) %>%
  ...
  mutate(Rail_Length = ifelse(is.na(Rail_Length)==TRUE, mean(Rail_Length,na.rm=TRUE),Rail_Length)) %>% #Puis complément par la moyenne des données existantes sur les données manquantes (p. ex. considère la valeur comme stable sur la période si une seule valeur)
  mutate(Rail_Density = Rail_Length/LandArea_Total) %>% #Puis complément par la moyenne des données existantes sur les données manquantes (p. ex. considère la valeur comme stable sur la période si une seule valeur)
  #Et Air et Train passengers, inter et extra polation à partir de la population
...
WB_DataBase_2[which(WB_DataBase_2$Country_Name=="Japan"&WB_DataBase_2$Year=="2018"),"Train_Passengers_new"]<-WB_DataBase_2[which(WB_DataBase_2$Country_Name=="Japan"&WB_DataBase_2$Year=="2017"),"Train_Passengers"] #Correction à la marge


#Possibilité de tracer les graphiques pour contrôler les imputations d'income/capita
if (Display_RegressionGraphs == "yes"){
  #On crée une variable pour distinguer les valeurs estimées des valeurs originales (pour income_pc)  
    WB_DataBase_2 <- WB_DataBase_2 %>%
    mutate(Original=ifelse(is.na(Income_pc)==TRUE,'Estimated', 'Original'))
  #On trace un graphe des valeurs estimées et originales par pays, en faisant apparaitre la régression non paramétrique utilisée pour une partie des imputations
    ...
    theme_classic() + theme(legend.position='none') + ggtitle("Graphique par pays avec les valeurs estimées")
  #On trace le même graphe en visualisant les valeurs estimées
    ggplot(WB_DataBase_2, aes(x=GDP_ConstantPrice_pc_new, y=Income_pc_new))+ 
    geom_point(aes(colour = Original))+geom_smooth(method = "loess", span = 2)+  
    theme_classic()  + ggtitle("Visualisation des valeurs estimées")
}



## 1.2 - Chargement et prétraitement des données d'activité de l'IEA

# Chargement des onglets utiles du fichier de l'IEA (ATTENTION, comme mentionné en haut : conversion en xlsx nécessaire au préalable)
setwd("~/EnergyEfficiencyIndicators/data")
IEA_Activity<-read.xlsx("IEA_EEI_database_Full_Rev.xlsx",sheet="Activity data") #Corrigé dans le fichier excel : espaces dans la colonne Product
IEA_Activity[which(IEA_Activity$Country=="Romania"&IEA_Activity$Product=="Population (10^6)"),"2000"] <- 22.44 #Correction d'une valeur aberrante à partir de la donnée de la banque mondiales
...
# Prétraitement des données de pkm IEA : sélection des variables, interpolations pour les données manquantes entre données existantes
PKM_AllModes<-IEA_Activity %>%
  ...
  mutate(Year=as.numeric(Year))  %>%
  group_by(Country) %>%
  mutate(Population_IEA=na.approx(Population_IEA,na.rm=FALSE))  %>% #On interpole (au cas où) les données de population (base : année)
  mutate(pkm_cars=na.approx(pkm_cars,x=Population_IEA,na.rm=FALSE))  %>% #On complète les données manquantes par interpolations en fonction de la taille de la population pour les 5 modes
  mutate(pkm_moto=na.approx(pkm_moto,x=Population_IEA,na.rm=FALSE))  %>% #NB : cette interpolation prime donc sur l'estimation des valeurs manquantes par régression (lorsqu'elles sont comprises entre deux données existantes)
  ...
  filter(Year!=2019)# %>% #On enlève l'année 2019 pour laquelle il y a beaucoup de données manquantes PIB
  

#On complète d'abord les séries incomplètes pour les pays IEA, pour n'avoir qu'un seul type d'imputation une fois qu'on reprend la base WB
PKM_AllModes_aug <- left_join (PKM_AllModes,WB_DataBase_2, by = c("Country" = "Country_Name", "Year"))  %>%
  group_by(Country) %>% #Ci-dessous, on complète les données manquantes aux extrémités de séries temporelles (relativement peu nombreuses)
  mutate(check_NA_cars=ifelse(sum(is.na(pkm_cars_pc))==19,0,ifelse(sum(is.na(pkm_cars_pc))==18,1,ifelse(sum(is.na(pkm_cars_pc))>=1,2,3))))  %>% #On diagnostique le nombre de NA ; 0 si que des NA ; 1 si une valeur existante ; 2 si au moins deux valeurs existantes (mais au moins une donnée manquante) ; 3 si aucune valeur manquante.
...
############ Réalisation des tests pour identifier les variables pertinentes de régression ############
...
# 1.3 - usage des modèles de régression retenus pour  :
...
lm.pkm.train3<-lm(pkm_train_pc~Income_pc_new+Population_Density, PKM_AllModes_aug)
lm.pkm.air6<-lm(pkm_air_pc~Income_pc_new+Year, PKM_AllModes_aug[which(PKM_AllModes_aug$LandArea_Total>5000000),]) #On utilise deux modèles prédictifs, l'un pour les grands pays (performant)
lm.pkm.air7<-lm(pkm_air_pc~Income_pc_new+Year, PKM_AllModes_aug[which(PKM_AllModes_aug$LandArea_Total<=5000000),]) #Et l'autre pour le reste du monde (très médiocre, mais qui prend une valeur moyenne), sachant qu'on ne trouve pas de modèle performant avec les données disponibles

#On crée une base intermédiaire qui permettra d'ajouter les variables issues de l'IEA à la base de tous les pays
...
#Pour finir, on choisit entre les valeurs estimées par le modèle et les valeurs données par l'IEA et leurs inter et extrapolations quand elles sont disponibles
WB_DataBase_aug3 <- WB_DataBase_aug %>%
  ...
  mutate (pkm_air_pc_new = ifelse(pkm_air_pc_new<=0, 0, pkm_air_pc_new)) # on empêche les valeurs négatives, sachant que la valeur minimale dans les données IEA pour cette variable est 0
...
## 2.1 On prépare la base IEA et construit la variable de taux d'occupation à partir de cette base
OccupancyRate_IEA<-IEA_Activity %>%
  ...
  filter(Country!='Armenia') %>% # (valeurs inférieures à 1 pkm/vkm)
...
##2.2 On estime un modèle de régression à partir de cette base, on l'applique et on finit par choisir la variable de l'IEA ou la variable estimée (si IEA non disponible)
...
if (Display_RegressionGraphs == "yes"){ #Pour avoir une idée de la régression utilisée (décroissante jusqu'à 60000$/cap, puis presque stable)
ggplot(OccupancyRate_IEA2, aes(x=Income_pc_new, y=OccupancyRate_cars))+ #pkm.moto.2014+
  geom_point(aes(colour = Country))+geom_smooth(method = "loess", span = 2)+  #method = "gam" #formula = y ~ splines::bs(x, 3), se = FALSE
  theme_classic() 
ggplot(OccupancyRate_IEA2, aes(x=Income_pc_new, y=OccupancyRate_moto))+ #pour la moto, on utilise la moyenne plutôt qu'une régression... celle-ci aurait pu être intéressante si nous avions eu des valeurs pour des pays à bas revenu, mais ce n'est pas le cas, et nous avons une très forte variabilité globalement
...
WB_DataBase_aug4 <- left_join(WB_DataBase_aug3, OccupancyRate_IEA2[,c("Country", "Year","OccupancyRate_cars","OccupancyRate_moto")], by = c("Country_Name" = "Country", "Year"))  %>%
  ...
  select(-OccupancyRate_cars) %>% #On nettoie la base pour éviter la confusion ou l'emploi d'une mauvaise variable
  select(-OccupancyRate_moto) %>%
  select(-occupancy.rate.est_cars) %>%
  select(-occupancy.rate.est_moto)

#On établit la moyenne qui sera utile ultérieurement
...
### 3. Variables d'efficacité énergétique et de consommation d'énergie

EnergyTransport_AllModes <- IEA_EnergyDemandTransport %>%
  rename(Mode='Mode/vehicle.type')%>%
  filter(Mode %in% c("Cars/light trucks","Motorcycles"))  %>% #unité : PJ
...
## Pour info, ci-dessous les fonctions de régression retenues pour respectivement cars et motorcycles 
###On constate que les consommations d'énergie pour les motos pour le gaz et l'électricité sont nulles
##Finalement pour le gaz et l'élec auto, on prendra la valeur moyenne
##Pour la moto on prendre plutôt la régression lm
if (Display_RegressionGraphs == "yes"){
...
#Estimation des efficacités par source et par pays
...
lm.eff.moto.ET<-lm(eff_moto_ET_vkm~Income_pc_new, EnergyTransport_AllModes_3) #NB : pour les motos, on considère un taux d'occupation de 1
loess.eff.moto.gas<-loess(eff_moto_gas~Income_pc_new, EnergyTransport_AllModes_3, span = 2,control = loess.control(surface = "direct")) #nul dans le jeu de données utilisé
loess.eff.moto.elec<-loess(eff_moto_elec~Income_pc_new, EnergyTransport_AllModes_3, span = 2,control = loess.control(surface = "direct")) #nul dans le jeu de données utilisé
#eff_moto_ET_vkm_min<-min(EnergyTransport_AllModes_3$eff_moto_ET_vkm,na.rm=TRUE) #On remarque que la régression permet des valeurs négatives, on utilise la valeur minimale de la base IEA comme valeur seuil (pour les valeurs estimées inférieures) - finalement remplacé par la moyenne


#Utilisation des modèles puis choix de la valeur IEA quand disponible, plutôt que la valeur estimée
...
WB_DataBase_aug5 <- WB_DataBase_aug5_temp %>%
  ...
  mutate(eff.cars.gas.est = mean(eff_cars_gas_vkm, na.rm=TRUE, w=Population_WB*pkm_cars_pc_new)) %>%  # on choisit d'établir l'estimation à la moyenne pondérée des valeurs par pays, la régression n'étant pas convaincante
  mutate(eff.cars.elec.est = 0) %>% # on choisit d'établir l'estimation à 0 ; les régressions étant trop dépendantes des cas particuliers sur 2000-2018
  mutate(eff.moto.ET.est = mean(eff_moto_ET_vkm, na.rm=TRUE, w=Population_WB*pkm_moto_pc_new)) %>%
  mutate(eff.moto.gas.est = predict(loess.eff.moto.gas, Income_pc_new, se = FALSE)) %>% # = 0 (avec les données 2000-2018)
  mutate(eff.moto.elec.est = predict(loess.eff.moto.elec, Income_pc_new, se = FALSE)) %>%  # = 0  (avec les données 2000-2018)
  ...
  #mutate(eff_moto_ET_vkm_new=ifelse(eff_moto_ET_vkm_new-eff_moto_vkm_min<=0,eff_moto_ET_vkm_min,eff_moto_ET_vkm_new)) %>% #Correction des valeurs estimées très basses, voire négatives dues à la régression - inutile puisqu'on a pris finalement la moyenne
  mutate(eff_moto_gas_vkm_new=ifelse(is.na(eff_moto_gas)==TRUE, eff.moto.gas.est, eff_moto_gas_vkm)) %>%
  mutate(eff_moto_gas_vkm_new=ifelse(eff_moto_gas_vkm_new<0,0,eff_moto_gas_vkm_new)) %>% #Correction des valeurs estimées très basses, voire négatives dues à la régression ; inutile sur les données initiales
  mutate(eff_moto_elec_vkm_new=ifelse(is.na(eff_moto_elec)==TRUE, eff.moto.elec.est, eff_moto_elec_vkm)) %>%
  mutate(eff_moto_elec_vkm_new=ifelse(eff_moto_elec_vkm_new<0,0,eff_moto_elec_vkm_new)) %>% #Correction des valeurs estimées très basses, voire négatives dues à la régression ; inutile sur les données initiales
...
#On corrige plusieurs variables pour la Chine, à partir de plusieurs sources de données (en particulier consommation de carburant - sur lequel on se base pour corriger aussi pkm cars et moto - et pkm rail et air)
#WB_DataBase_aug5_copy<-WB_DataBase_aug5 - ligne pour vérifier
setwd("~/Transport")
Correctif_PKM_China<-read.xlsx("Correction_PKM_China_fromOtherSources.xlsx",sheet="China") #sur la période 2000-2018
#WB_DataBase_aug5[which(WB_DataBase_aug5$Country_Name=="China"),c("OccupancyRate_new")]<-WB_DataBase_aug5[which(WB_DataBase_aug5$Country_Name=="China"),c("OccupancyRate_new")]/Correctif_PKM_China[,"China.Gasoline.Demand.PJ"]*WB_DataBase_aug5[which(WB_DataBase_aug5$Country_Name=="China"),c("Cons_Hsld_PrivateModes")] #On choisit plutôt de corriger les pkm que le taux d'occupation
...
#View(WB_DataBase_aug5_copy[which(WB_DataBase_aug5$Country_Name=="China"),]) # - ligne pour vérifier
#View(WB_DataBase_aug5[which(WB_DataBase_aug5$Country_Name=="China"),]) # - ligne pour vérifier
...
WB_DataBase_final_Reg_temp <- left_join(WB_DataBase_final,CountryRegion,by=c("Country_Code"="ISO3")) %>%
  ...
  summarise(Income_pc_new = mean(Income_pc_new, na.rm=TRUE, w=Population_WB),
            OccupancyRate_cars = mean(OccupancyRate_cars_new, na.rm=TRUE, w=(Population_WB*pkm_cars_pc_new)), #la pondération pourrait être faite sur vkm
            OccupancyRate_moto = mean(OccupancyRate_moto_new, na.rm=TRUE, w=(Population_WB*pkm_moto_pc_new)), 
            eff_cars_ET_vkm = mean(eff_cars_ET_vkm_new, na.rm=TRUE, w=(pkm_cars_pc_new*Population_WB))*Convert_EnergyUnit, #pondération sur les pkm
            ...
            eff_moto_ET_vkm = mean(eff_moto_ET_vkm_new, na.rm=TRUE, w=(pkm_moto_pc_new*Population_WB))*Convert_EnergyUnit, #pondération sur les pkm
  ...
  mutate(ID_reg_Im=Im_region) %>% #On crée un code pour réordonner les lignes selon l'ordre des régions Imaclim-R monde
...
Correctif_AirTravel_ICAO<-read.xlsx("~/Transport/AirTransport_ShareDomestic.xlsx",sheet="ICAO") #sur l'année 2014, on corrige les données de demande de passagers.km à l'aide de données de l'ICAO. Le problème était notamment que seuls les vols domestiques (internes à une région) sont pris en compte. Par contre, on ne distingue pas les voyages de tourisme des voyages d'affaire (pas de statistiques identifiées, il faudra produire une estimation à partir de DF/CI en service de transport ; sachant que les prix pratiqués pour le business sont normalement plus élevés - https://www.investopedia.com/ask/answers/041315/how-much-revenue-airline-industry-comes-business-travelers-compared-leisure-travelers.asp)

if (LoadGTAP_forAirCorrection == 'yes') { #Ci-dessous, pour compléter le correctif, on utilise les données GTAP et la demande en air transport DF / (DF + CI) pour approcher la part des pkm AIR réalisée par les ménages
setwd("~/")
...
#On crée le vecteur final de cette part pour les 12 régions
atp_shareDF<-as.data.frame(atp_DF/(atp_DF+atp_CI*Correctif_AirTravel_ICAO[1:12,'Im_estimation_sharePassenger_2014']),row.names = rownames(DF_dom)) %>% rename (share_DF_in_atp = everything()) #On introduit également ici l'estimation de VA du secteur fret vs passagers
atp_shareDF['USA',]<-atp_shareDF['CAN',]  #Il semble y avoir une différence de comptabilité entre USA et les autres pays : très faible DF dans le secteur de l'aviation, par contre très forte DF de recreational other services, qui a lui même un input très élevé d'aviation. On utilise donc le ratio du canada (60%, au lieu de 5% avec la méthode précédente)
atp_shareDF['CHN',]<-atp_shareDF['CIS',]  #De même, la valeur pour la Chine sort du lot (~11%). Je n'ai rien trouvé dans la littérature sur une spécificité Chinoise sur le sujet, j'affecte donc la valeur minimale du reste des régions (CIS = 37%)
}
...
#On va recalculer une efficacité en tenant compte de la moto
...
#Ci-dessous quelques graphiques de vérification des ordres de grandeur des valeurs
if (Display_RegressionGraphs == "yes") {
...
#Liste de pays à valeurs aberrantes à traiter 
#Kuwait et United Arab Emirates : valeurs très élevées de revenu par capita ; préférer le calcul à partir du GDP/cap plutôt que l'extrapolation (car données très variables)
#Nauru : PIB négatif
#Income/cap négatif, du fait de l'imputation : Afganistan, Chad, Ethiopia, Nauru, Myanma, Timor-Leste, Myanmar
#et dans les données d'origine Equatorial Guinea (pour l'année 2005)
}
