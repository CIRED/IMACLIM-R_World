...
//9: Offshore Wind
        if current_time_im ==1 
            for i=1:TimeHorizon
                ...
                disc_rate_elec(:,technoElecNuke,i) = repmat(Global_WACC(:,3),1,length(technoElecNuke));
    ...
    case 2 //convergence case
        if current_time_im>=wacc_conv_start & current_time_im<=wacc_conv_end
            for i = current_time_im:TimeHorizon
                ...
                disc_rate_elec(:,technoElecNuke,i) = conv_WACC(technoElecNuke,wacc_min,weight_init,weight_end,3);
