...
for lineVAR =1:nbLines //[1:16 35:124 126:293 302:nbLines]
    ...
    if strindex( VARname, 'Price') == [] & strindex( VARname, 'Capital Cost') == [] 
...
// but with exceptions
list_techind_temp=[indice_CSP,indice_GGS, indice_WNO, indice_BIS, indice_BIG, indice_SBI];
for vary=['Capital Cost',"Market shares"]
    indy=0;
...
lineVAR = find_index_list( list_output_str, "Curtailment|Share"); // 
weightLine = find_index_list( list_output_str, "Secondary Energy|Electricity");
outputs_temp(nbLines*reg+lineVAR,ind_time) = divide( sum(outputs_temp(nbLines*[0:11] + weightLine,ind_time) .* outputs_temp(nbLines*[0:11] + lineVAR,ind_time)), sum(outputs_temp(nbLines*[0:11] + weightLine,ind_time)), 0);

lineVAR = find_index_list( list_output_str, "Storage|Share"); // 
weightLine = find_index_list( list_output_str, "Secondary Energy|Electricity");
outputs_temp(nbLines*reg+lineVAR,ind_time) = divide( sum(outputs_temp(nbLines*[0:11] + weightLine,ind_time) .* outputs_temp(nbLines*[0:11] + lineVAR,ind_time)), sum(outputs_temp(nbLines*[0:11] + weightLine,ind_time)), 0);

lineVAR = find_index_list( list_output_str, 'Electricity|Residual System CF'); // 
weightLine = find_index_list( list_output_str, "Secondary Energy|Electricity");
outputs_temp(nbLines*reg+lineVAR,ind_time) = divide( sum(outputs_temp(nbLines*[0:11] + weightLine,ind_time) .* outputs_temp(nbLines*[0:11] + lineVAR,ind_time)), sum(outputs_temp(nbLines*[0:11] + weightLine,ind_time)), 0);
