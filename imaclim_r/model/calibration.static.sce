...
exec(MODEL+"calibration.population.sce");
...
exec(common_codes_head_dir+"readGtapData.sci");
...
exec(MODEL+"calibration.laborProductivity.sce");
...
l=zeros(reg,sec);	// unitary labor requirements

// data from BIT statistics for 5 sectors: Fossil Fuels, Elecricity/gas, Construction, Services, Transport
...
ind_temp_specific = [indice_coal,indice_gas:indice_elec];
...
partsalFF=SALNET(:,1:3)./(sum(SALNET(:,1:3),'c')*ones(1,3));	// repartition based on share of wages in fossiel fuels sectors
partsalEG=SALNET(:,4:5)./(sum(SALNET(:,4:5),'c')*ones(1,2));	// same for liquids and electricity
partsalTR=SALNET(:,8:10)./(sum(SALNET(:,8:10),'c')*ones(1,3));	// same for transport sectors

////desagregation in 3 productive sectors
// repartition based on share of wages in the three productive sectors
partsalCOMP=SALNET(:,indice_composite)	./(SALNET(:,indice_composite)+SALNET(:,indice_agriculture)+sum(SALNET(:,indice_industries),'c'));
partsalAGRI=SALNET(:,indice_agriculture)	./(SALNET(:,indice_composite)+SALNET(:,indice_agriculture)+sum(SALNET(:,indice_industries),'c'));
partsalIND=SALNET(:,indice_industries)	./repmat((SALNET(:,indice_composite)+SALNET(:,indice_agriculture)+sum(SALNET(:,indice_industries),'c')),1,nb_sectors_industry);

// wage repartition among sectors
...
labor_ILO = csvRead(path_employment+'labor.csv','|',[],[],[],'/\/\//');
labor_ILO = evstr(labor_ILO(2:$,2:$))*1e3;
labor_ILO0 = labor_ILO;

/////////////////////// l = unitary labor requirements
l=partLsec.*((L.*(1-z))*ones(1,sec))./Qref;	
lref = l;

wref=SALNET./(l.*Qref.*(aRD+bRD.*tanh(cRD.*(Qref./Cap-ones(reg,sec))))); // unitary wage
