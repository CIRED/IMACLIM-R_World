...
if do_smooth_outputs==%t
    execstr("sel_outputs_"+strsubst( ETUDEOUTPUT,'.','')+" = yearlySmoothOutputs(:, year_to_select)");
else
    execstr("sel_outputs_"+strsubst( ETUDEOUTPUT,'.','')+" = yearlyOutputs(1:(13*nbLines), year_to_select)");
end
mkcsv("sel_outputs_"+strsubst( ETUDEOUTPUT,'.',''));
mkcsv("sel_outputs_"+strsubst( ETUDEOUTPUT,'.',''),OUTPUT);
