...
counterLine =counterLine+ 1; if current_time==yr_start & k==1; list_output_str($+1) = varname; end;
outputs_temp(nbLines*(k-1)+counterLine,current_time+1) = glob_in_bioelec_Et_reg(k) / Exajoule2Mkcal + unused_other_agrof_prod(k).*Mkcal2Exajoule ;
if current_time==yr_start & k==1; list_output_unit($+1) = "EJ/yr";
end;
...
outputs_temp(nbLines*(k-1)+counterLine,current_time+1) = outputs_temp(nbLines*(k-1)+counterLine,current_time+1) + reg_natfor_4Ener(k).*Mkcal2Exajoule; //Other <- Managed forest
...
varname_tpt = 'Final Energy|Other Sector|Solids'; //    EJ/yr
lineVAR = find_index_list( list_output_str, 'Primary Energy|Biomass|Traditional');
counterLine =counterLine+ 1; //if current_time==yr_start & k==1; list_output_str($+1) = varname_tpt; end;
...
//if current_time==yr_start & k==1; list_output_unit($+1) = "EJ/yr";
//end;
//if current_time==yr_start & k==1;list_output_comments(counterLine)="Consists of coal and traditional biomass. The use of traditional biomass as solids is not modelled in Imaclim-R, but reported consistently according to the traditional biomass produced in the Nexus Land-Use model";end;
...
counterLine =counterLine+ 1; if current_time==yr_start & k==1; list_output_str($+1) = varname; end;
outputs_temp(nbLines*(k-1)+counterLine,current_time+1) = unused_other_agrof_prod(k).*Mkcal2Exajoule;
if current_time==yr_start & k==1; list_output_unit($+1) = "EJ/yr";
end;
...
counterLine =counterLine+ 1; if current_time==yr_start & k==1; list_output_str($+1) = varname; end;
lineVAR = find_index_list( list_output_str, 'Primary Energy|Biomass|Electricity');
lineVAR1 = find_index_list( list_output_str, 'Primary Energy|Biomass|Liquids');
lineVAR2 = find_index_list( list_output_str, 'Primary Energy|Biomass|Hydrogen');
lineVAR3 = find_index_list( list_output_str, 'Primary Energy|Biomass|Traditional');
...
varname = 'Price|Agriculture|Corn|Index'; // Index (2010 = 1)
counterLine =counterLine+ 1; if current_time==yr_start & k==1; list_output_str($+1) = varname; end;
outputs_temp(nbLines*(k-1)+counterLine,current_time+1) = %nan;
if current_time==yr_start & k==1; list_output_unit($+1) = "Index (2010=1)";
end;


varname = 'Price|Agriculture|Non-Energy Crops|Index'; // Index (2010 = 1)
counterLine =counterLine+ 1; if current_time==yr_start & k==1; list_output_str($+1) = varname; end;
outputs_temp(nbLines*(k-1)+counterLine,current_time+1) = pcalveg(k);
if current_time==yr_start & k==1; list_output_unit($+1) = "Index (2010=1)";
end;

varname = 'Price|Agriculture|Soybean|Index'; // Index (2010 = 1)
counterLine =counterLine+ 1; if current_time==yr_start & k==1; list_output_str($+1) = varname; end;
outputs_temp(nbLines*(k-1)+counterLine,current_time+1) = %nan;
if current_time==yr_start & k==1; list_output_unit($+1) = "Index (2010=1)";
end;

varname = 'Price|Agriculture|Wheat|Index'; // Index (2010 = 1)
counterLine =counterLine+ 1; if current_time==yr_start & k==1; list_output_str($+1) = varname; end;
outputs_temp(nbLines*(k-1)+counterLine,current_time+1) = %nan;
if current_time==yr_start & k==1; list_output_unit($+1) = "Index (2010=1)";
end;
...
if real_failed
    ...
    lineVAR_otherfeed = find_index_list( list_output_str, 'Primary Energy|Biomass|Solids');
...
if current_time==yr_start & k==1; list_output_str($+1) = varname_driver; end;
outputs_temp(nbLines*13+counter_NLU*12+k,current_time+1) = va_agri(k) * usd_year1_year2;
if current_time==yr_start & k==1; list_output_unit($+1) = "billion US$2010/yr";
end;
...
// TODO : eventualy report
//Agricultural Demand|Livestock in million t DM/yr
//Agricultural Demand|Livestock|Food in million t DM/yr
