...
if indice_building_electri==1 & current_time_im >= start_year_strong_policy-base_year_simulation 
	...
	shCIspec_comp(indice_gaz,:)=0.05*ones(1,nb_regions); // Optimistic assumption: allows a higher electrification rate in the commercial sector
...
for pays=1:reg
    ...
    p_ener_composite=[pArmCI_nexus(indice_coal,indice_composite,pays);pArmCI_nexus(indice_Et,indice_composite,pays);pArmCI_nexus(indice_gaz,indice_composite,pays);pArmCI_nexus(indice_elec,indice_composite,pays)];
...
//les deux lignes suivantes sont utiles en cas de pause. Voir http://wiki.scilab.org/howto/global_and_local_variables
...
for k=1:reg
    ...
    CIdeltacompositeSubs(indice_gaz,indice_composite,k)=E_enerdelta_ser(k)*CIcompositeSubsref(indice_gaz,indice_composite,k)*fmultPgaz_comp_Edelta(k)*fmult_savingfuelswit_ser(k);
...
for k=1:reg
    CIdeltacompositeSpec(indice_gaz,indice_composite,k)=min(E_enerdelta_ser(k)*CIcompositeSpecref(indice_gaz,indice_composite,k),...
    E_enerdelta_ser(k)*CIcompositeSpecref(indice_gaz,indice_composite,k)*(pArmCI_nexus(indice_gaz,indice_composite,k)/pArmCIref(indice_gaz,indice_composite,k))^elast_gaz_comp(k));
...
///CI totales des nouvelles capacités
for k=1:reg
    ...
    CIdeltacomposite(indice_gaz,indice_composite,k)=CIdeltacompositeSubs(indice_gaz,indice_composite,k)+CIdeltacompositeSpec(indice_gaz,indice_composite,k); 
    ...
    CIdeltacomposite(indice_gaz,indice_composite,k)=CIdeltacompositeSubs(indice_gaz,indice_composite,k)+CIdeltacompositeSpec(indice_gaz,indice_composite,k)+CIdeltacompCoalToGaz(k);
...
for pays=1:reg
    ...
    p_ener_agriculture=[pArmCI_nexus(indice_coal,indice_agriculture,pays);pArmCI_nexus(indice_Et,indice_agriculture,pays);pArmCI_nexus(indice_gaz,indice_agriculture,pays);pArmCI_nexus(indice_elec,indice_agriculture,pays)];
...
///CI substituables des nouvelles capacités
for k=1:reg
    ...
    CIdeltacompositeSubs(indice_gaz,indice_agriculture,k)=E_enerdelta_agr(k)*CIagricSubsref(indice_gaz,indice_agriculture,k)*fmultPgaz_agric_Edelta(k);
...
///CI spécifiques des nouvelles capacités
for k=1:reg

    // Pour le gaz, on diminue les CIspec après 2030
    CIdeltacompositeSpec(indice_gaz,indice_agriculture,k)=min(E_enerdelta_agr(k)*CIagricSpecref(indice_gaz,indice_agriculture,k),...
    E_enerdelta_agr(k)*CIagricSpecref(indice_gaz,indice_agriculture,k)*(pArmCI_nexus(indice_gaz,indice_agriculture,k)/pArmCIref(indice_gaz,indice_agriculture,k))^elast_gaz_agr(k)) ;

    // Pour Et, évolution comme CI_delta_Et_OT avec l'EEI en plus.
    ...
    // Substitution autonome du charbon vers le gaz
    if current_time_im==1
...
for k=1:reg
    ...
    CIdeltacomposite(indice_gaz,indice_agriculture,k)=CIdeltacompositeSubs(indice_gaz,indice_agriculture,k)+CIdeltacompositeSpec(indice_gaz,indice_agriculture,k)+CIdeltaagriCoalToGaz(k);
...
for i = 1:nb_sectors_industry,
	for pays=1:reg
		...
		p_ener_industrie=[pArmCI_wo_CCS_nexus(indice_coal,indice_industries(i),pays);pArmCI_wo_CCS_nexus(indice_Et,indice_industries(i),pays);pArmCI_wo_CCS_nexus(indice_gaz,indice_industries(i),pays);pArmCI_wo_CCS_nexus(indice_elec,indice_industries(i),pays)];		
		p_ener_industrie_CCS=[pArmCI_w_CCS_nexus(indice_coal,indice_industries(i),pays);pArmCI_w_CCS_nexus(indice_Et,indice_industries(i),pays);pArmCI_w_CCS_nexus(indice_gaz,indice_industries(i),pays);pArmCI_w_CCS_nexus(indice_elec,indice_industries(i),pays)];
		LCC_industrie_noCCS=K_cost_industries(pays,:,i)'.*CRF_industrie+p_ener_industrie./rhoindustrie_dyn;
		LCC_industrie_CCS=K_cost_industries(pays,:,i)'.*CRF_industrie+p_ener_industrie_CCS./rhoindustrie_dyn + CCS_cost_industrie .* croyance_taxe(pays)/1e6*coef_Q_CO2_CI_prev([indice_coal,indice_Et,indice_gaz,indice_elec],indice_industries(i),pays);
		...
		if ind_electrification_indu<>2 & current_time_im >=start_year_strong_policy-base_year_simulation
			pArmCI_nexus_industrie([indice_coal,indice_Et,indice_gaz,indice_elec],indice_industries(i),pays) = choose_CCS_industrie(:,pays).*pArmCI_wo_CCS_nexus([indice_coal,indice_Et,indice_gaz,indice_elec],indice_industries(i),pays) +(~choose_CCS_industrie(:,pays)) .* pArmCI_w_CCS_nexus([indice_coal,indice_Et,indice_gaz,indice_elec],indice_industries(i),pays);
		else
			pArmCI_nexus_industrie([indice_coal,indice_Et,indice_gaz,indice_elec],indice_industries(i),pays) = pArmCI_wo_CCS_nexus([indice_coal,indice_Et,indice_gaz,indice_elec],indice_industries(i),pays);
		end
	end
...
for k=1:reg
    ...
    CIdeltacompositeSubs(indice_gaz,indice_industries,k)	=E_enerdelta_ind(k,:).*CIindSubsref(indice_gaz,indice_industries,k)	.*fmultPgaz_ind_Edelta(k,:);
...
///CI spécifiques des nouvelles capacités
for k=1:reg
    CIdeltacompositeSpec(indice_gaz,indice_industries,k)=min (E_enerdelta_ind(k,:).*CIindSpecref(indice_gaz,indice_industries,k),...
    E_enerdelta_ind(k,:).*CIindSpecref(indice_gaz,indice_industries,k).*(pArmCI_nexus_industrie(indice_gaz,indice_industries,k)./pArmCIref(indice_gaz,indice_industries,k)).^elast_gaz_ind(k,:));
...
for k=1:reg
    ...
    CIdeltacomposite(indice_gaz,indice_industries,k)	=CIdeltacompositeSubs(indice_gaz,indice_industries,k)+CIdeltacompositeSpec(indice_gaz,indice_industries,k);
...
////////////////////////////////////////////////////////////////////////////////////////
//////// EVALUATION DU COUT EN CAPITAL DE L'EFFICACITE INDUITE PAR LA TAXE CARBONE /////
//calcul des Caractéristiques technologiques des capacités construites sans efficacité induite
...
for k=1:reg
    ...
    CIdeltacompSubs_BAU(indice_gaz,indice_composite,k)=E_enerdelta_ser_BAU(k)* CIcompositeSubsref(indice_gaz, indice_composite,k) * fmultPgaz_comp_Edelta(k);
    CIdeltacompSpec_BAU(indice_gaz,indice_composite,k)=E_enerdelta_ser_BAU(k)* CIcompositeSpecref(indice_gaz, indice_composite,k);
    CIdeltacomposite_BAU(indice_gaz,indice_composite,k)=CIdeltacompSubs_BAU(indice_gaz,indice_composite,k)+CIdeltacompSpec_BAU(indice_gaz,indice_composite,k);
...
for k=1:reg
    ...
    CIdeltacompSubs_BAU(indice_gaz,indice_agriculture,k)=E_enerdelta_agr_BAU(k)* CIagricSubsref(indice_gaz, indice_agriculture,k) * fmultPgaz_agric_Edelta(k);
    CIdeltacompSpec_BAU(indice_gaz,indice_agriculture,k)=E_enerdelta_agr_BAU(k)* CIagricSpecref(indice_gaz, indice_agriculture,k);
    CIdeltacomposite_BAU(indice_gaz,indice_agriculture,k)=CIdeltacompSubs_BAU(indice_gaz,indice_agriculture,k)+CIdeltacompSpec_BAU(indice_gaz,indice_agriculture,k);
...
for k=1:reg
	...
	CIdeltacompSubs_BAU(indice_gaz,indice_industries,k)=E_enerdelta_ind_BAU(k,:)	.* CIindSubsref(indice_gaz, indice_industries,k) .* fmultPgaz_ind_Edelta(k,:);
    CIdeltacompSpec_BAU(indice_gaz,indice_industries,k)=E_enerdelta_ind_BAU(k,:)	.* CIindSpecref(indice_gaz, indice_industries,k);
    CIdeltacomposite_BAU(indice_gaz,indice_industries,k)=CIdeltacompSubs_BAU(indice_gaz,indice_industries,k)+CIdeltacompSpec_BAU(indice_gaz,indice_industries,k);
...
//////////////////////////initialisation de la nouvelle generation de capital pour le composite
...
for k=1:reg
	...
	for i=1:nb_sectors_industry
		for j=1:dureedevieindustrie
			sumEmi_temp = sumEmi_temp + (~CCS_in_industrie_vintage(:,k,i,current_time_im+j)) .* coef_Q_CO2_CI_wo_CCS( [indice_coal,indice_Et,indice_gaz,indice_elec],indice_industries(i),k) .* Capvintageindustries(k,i,current_time_im+j);
			sumEmi_temp = sumEmi_temp + (CCS_in_industrie_vintage(:,k,i,current_time_im+j)) .* (1-CCS_efficiency_industry) .* coef_Q_CO2_CI_wo_CCS( [indice_coal,indice_Et,indice_gaz,indice_elec],indice_industries(i),k) .* Capvintageindustries(k,i,current_time_im+j);
			sumCap_temp = sumCap_temp + Capvintageindustries(k,i,current_time_im+j);
		end
    coef_Q_CO2_CI( [indice_coal,indice_Et,indice_gaz,indice_elec],indice_industries(i),k) = sumEmi_temp / sumCap_temp;
	end
end
...
for k=1:reg
	for i=1:nb_sectors_industry
		...
		*max(partDomDF(k,indice_industries(i)),0.000000000000000000001)*DF(k,indice_industries(i))/(max(partDomDFref(k,indice_industries(i)),0.000000000000000000001)*DFref(k,indice_industries(i))));
	end
    ...
    *partDomDF(k,indice_agriculture)*DF(k,indice_agriculture)/(partDomDFref(k,indice_agriculture)*DFref(k,indice_agriculture)));
    //report sur les consommations de composite dans composite (hypothèse de coût constant)
