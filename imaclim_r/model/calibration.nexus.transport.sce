...
// number of modes of transport for households
nb_trans_mode_hsld = 3;
ind_hsld_transpAir = 1; 
ind_hsld_transpOT = 2; // this includes both rail and road passenger transportation
ind_hsld_transpCar = 3;
ind_hsld_transpNM = 4; // non-mothorized

// number and types of modes in Other transport
nb_desag_OT = 4;
indice_desag_OT = ["rail_freight" "rail_passenger" "road_freight" "road_passenger"]; // Warning: the order must correspond to the order in file containing data data_OT_varioussources.csv
...
betatrans=1/(nb_trans_mode_hsld+1)*ones(nb_regions,nb_trans_mode_hsld+1);//number of transport modes plus non-motorized

// share of households expenditures in composite goods spent on car maintenance
//In current version it is assumed that 10% of composite good consumption by household correspond to cars OM costs
//One improvement would be to recompose it also at the calibration year with actual data of OM costs used in calibration.nexus.cars
shCompoEntretienAuto=0.1*ones(nb_regions,1);//in dynamics this expenditure is recomposed with cars OM costs

// base 100 / reference for pkm automobile
Tautomobileref = 100*ones(nb_regions,1);


// base 100 / stock of vehicles
stockautomobileref = 100*ones(nb_regions,1);

// base 100 / infrastructures routières
stockinfraref=100*ones(nb_regions,1);
...
nb_hour_travelled = 1.1;
nb_days_peryear = 365;


//basic needs for transport modes
bnNM=zeros(nb_regions,1);
//default assumption for cars, 50% of pkm by cars at calibration date
ind_newBN = 0;
if ind_newBN == 0 | current_time_im == 2017-base_year_simulation 
bnautomobileref=50*ones(nb_regions,1);
else
...
// Other Transport and air basic need as a share of calibration year consumption
// default assumption: set to zero for all regions
bnOT_share_Cons = zeros(nb_regions,1);
bnair_share_Cons = zeros(nb_regions,1);

// evolution of basic needs over time. default assumption: basic needs for cars follow evolution in pkm by car (sort of lock-in or habituation)
...
roadFret_optim_efficienc=0.02;

// energy efficiency gains from speed reduction (and consequent fleet increase) for shipping - if ind_shipFret_A=1

// the treaty for speed reduction take 18 years to be put in force
// as decrbired by Faber et al., 2012: https://cedelft.eu/publications/regulated-slow-steaming-in-maritime-transport/
// we assume the shift starts 9 years before
// the assumption of 15% energy efficiency due to speed reduction if taken from
// Faber et al., 2012, consitent with Walsh et al., 2017: https://doi.org/10.1016/j.marpol.2017.04.019
...
// exogenous substitution rate of petroleum to electric vehicles in Other Transports - if ind_OT_electrification=1
...
// we assume the same efficiency  for eletric trucks
...
//Households energy use for private vehicles. Generate data comes from the hybridization procedure of Energy Balances and National Accounting Matrixes from GTAP and IEA
//part of which comes from the IEA Energy Efficiency database
...
//Private vehicles stocks
//Note this is defined here, even though it is related to cars (and not in dedicated calibration.nexus.cars) because it is needed before calibration.nexus.cars is run
//data source https://www.oica.net/category/vehicles-in-use/ (all vehicles)
nombreautomobileref = csvRead(path_transport_cars+'nombreautomobileref.csv','|',[],[],[],'/\/\//');
nb_car_pc=nombreautomobileref./Ltot0;

//////////////////////////////////////////////////////////////////////////////////////////////////////////////
/////////////////////////////////////////Computing number of kilometers done with those vehicles
///////////////////////////////to ensure consistency between number of vehicles and final energy use

pkm_ref = zeros( nb_regions, nb_trans_mode_hsld);
...
//Correctin pkm mobility for regions we corrected the energy intensity of pkm, in consistency with energy statistics
...
// Assumpton on base year utilization rate of transport capacities (pkm/Capacicites)
URtransportref = 0.5 * ones(nb_regions, nb_trans_mode_hsld);
...
//total travel time
Tdisp=Ltot.*( nb_hour_travelled * ones(nb_regions,1)) * nb_days_peryear;
...
btrans=ones(nb_regions,nb_trans_mode_hsld);
atrans = -ones(nb_regions,nb_trans_mode_hsld) + (1-decrease_ti_fullCapacity) * ( ones(nb_regions,1) * (1 ./marginal_eff_travelTime(ind_hsld_transpAir:ind_hsld_transpCar)'));

ktrans=zeros(nb_regions,nb_trans_mode_hsld);
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////Setting variables values that are defined with previously set values of parameters and variables
...
//unitary (per pkm) energy use for cars
//Note this is defined here, even though it is related to cars (and not in dedicated calibration.nexus.cars) because it is needed before calibration.nexus.cars is run
...
/////////////////////////////////////////////////////////
////////// Setting values of parameters or trends used for functions or exogenous evolutions in dynamic


/////////////////////////////////////////////////// For the evolution of motorization rates, growth of motorization per capita as a function of income per capita growth
//Values from MoMo model
inc_grwth_mult_A=csvRead(path_transport_car_unit+'income_growth_multipliers_cars_A.csv','|',[],[],[],'/\/\//');
...
///////////parameters for the variant where nexus.cars is not used and instead a very simplied representation of the car fleet evolution is made with a price elasticity
//taken from a run from POLES, see file "D:\12 secteurs 2100\Nexus automobile\couplage étape 3 VCC\Nexus automobile "
elast_auto_Et=-0.55*ones(nb_regions,1);//price elasticity
asympt_alphaEtauto=0.2*ones(nb_regions,1);//technical asymptote for minimum liquid fuel use
...
if indice_E==1
    ...
    elast_auto_Et=-0.55*ones(nb_regions,1);
elseif indice_E==0
...
if indice_A==1
    ...
    asympt_alphaEtauto=0.3*ones(nb_regions,1);
elseif indice_A==0
...
//--------------------------------------------------------------------//
// X. Other terrestrial transportation section (new version 12/2023   //
//--------------------------------------------------------------------//
// This new version aims to disaggregate the "Othe Transportation" sector into rail freight, rail passenger, road freight, road passenger. 
...
share_OT_FinalEnergy_ref(ind_chn,indice_desag_OT == "rail_passenger") = share_OT_FinalEnergy_ref(ind_chn,indice_desag_OT == "rail_passenger") + 0.03;
share_OT_FinalEnergy_ref(ind_chn,indice_desag_OT == "road_passenger") = share_OT_FinalEnergy_ref(ind_chn,indice_desag_OT == "road_passenger") - 0.03;
...
ratio_OT_fret_rail_ref = share_OT_EnergyServi_ref(:,indice_desag_OT == "rail_freight")./(share_OT_EnergyServi_ref(:,indice_desag_OT == "rail_freight")+share_OT_EnergyServi_ref(:,indice_desag_OT == "road_freight"));
ratio_OT_pass_rail_ref = share_OT_EnergyServi_ref(:,indice_desag_OT == "rail_passenger")./(share_OT_EnergyServi_ref(:,indice_desag_OT == "rail_passenger")+share_OT_EnergyServi_ref(:,indice_desag_OT == "road_passenger"));
...
convfactorOT_tkmtoDF = (Qref(:,indice_OT)-DFref(:,indice_OT)) ./ (value_OT_EnergyServi_ref(:,indice_desag_OT == "rail_freight") + value_OT_EnergyServi_ref(:,indice_desag_OT == "road_freight")); // Should "-Exp(:,indice_OT )+Imp(:,indice_OT )" be added to Q? => a priori no
convfactorOT_pkmtoDF = DFref(:,indice_OT) ./ (value_OT_EnergyServi_ref(:,indice_desag_OT == "rail_passenger") + value_OT_EnergyServi_ref(:,indice_desag_OT == "road_passenger"));
...
for k=1 : nb_regions
	...
	alpharail_freight_ref (k,indice_elec)	= Qref(k,indice_OT) .* CI_OT_serv(indice_elec,indice_OT,k) .*share_OT_FinalEnergy_ref(k,indice_desag_OT == "rail_freight")   ./(share_OT_FinalEnergy_ref(k,indice_desag_OT == "rail_passenger") + share_OT_FinalEnergy_ref(k,indice_desag_OT == "rail_freight")) ./ EnergyServices_rail_fret(k);
	alpharail_passenger_ref (k,indice_elec)	= Qref(k,indice_OT) .* CI_OT_serv(indice_elec,indice_OT,k) .*share_OT_FinalEnergy_ref(k,indice_desag_OT == "rail_passenger") ./(share_OT_FinalEnergy_ref(k,indice_desag_OT == "rail_passenger") + share_OT_FinalEnergy_ref(k,indice_desag_OT == "rail_freight")) ./ EnergyServices_rail_pass(k);

	// We assume that 100% of coal consumption in OT at the calibration year is for trains
	alpharail_freight_ref (k,indice_coal) 	= Qref(k,indice_OT) .* CI_OT_serv(indice_coal,indice_OT,k) .*share_OT_FinalEnergy_ref(k,indice_desag_OT == "rail_freight")   ./(share_OT_FinalEnergy_ref(k,indice_desag_OT == "rail_passenger") + share_OT_FinalEnergy_ref(k,indice_desag_OT == "rail_freight")) ./ EnergyServices_rail_fret(k);
	alpharail_passenger_ref (k,indice_coal)	= Qref(k,indice_OT) .* CI_OT_serv(indice_coal,indice_OT,k) .*share_OT_FinalEnergy_ref(k,indice_desag_OT == "rail_passenger") ./(share_OT_FinalEnergy_ref(k,indice_desag_OT == "rail_passenger") + share_OT_FinalEnergy_ref(k,indice_desag_OT == "rail_freight")) ./ EnergyServices_rail_pass(k);

	// We assume that 100% of gas consumption in OT is for buses
	alpharoad_passenger_ref (k,indice_gaz) 	= Qref(k,indice_OT) .* CI_OT_serv(indice_gaz,indice_OT,k)  ./ EnergyServices_road_pass(k); 
	...
	alpharail_freight_ref (k,indice_Et) 		= (sum(Qref(k,indice_OT) .* CI_OT_serv(energyIndexes,indice_OT,k) .*share_OT_FinalEnergy_ref(k,indice_desag_OT == "rail_freight")) ..  // Total final energy of this mode according to the share of FE
												- Qref(k,indice_OT) .* CI_OT_serv(indice_coal,indice_OT,k) .*share_OT_FinalEnergy_ref(k,indice_desag_OT == "rail_freight") ./(share_OT_FinalEnergy_ref(k,indice_desag_OT == "rail_passenger") + share_OT_FinalEnergy_ref(k,indice_desag_OT == "rail_freight"))  .. // - coal
												- Qref(k,indice_OT) .* CI_OT_serv(indice_elec,indice_OT,k) .*share_OT_FinalEnergy_ref(k,indice_desag_OT == "rail_freight") ./(share_OT_FinalEnergy_ref(k,indice_desag_OT == "rail_passenger") + share_OT_FinalEnergy_ref(k,indice_desag_OT == "rail_freight"))) .. // - elec
												./ EnergyServices_rail_fret(k);
												
	alpharail_passenger_ref (k,indice_Et) 	= (sum(Qref(k,indice_OT) .* CI_OT_serv(energyIndexes,indice_OT,k) .*share_OT_FinalEnergy_ref(k,indice_desag_OT == "rail_passenger")) ..  // Total final energy of this mode according to the share of FE
												- Qref(k,indice_OT) .* CI_OT_serv(indice_coal,indice_OT,k) .*share_OT_FinalEnergy_ref(k,indice_desag_OT == "rail_passenger") ./(share_OT_FinalEnergy_ref(k,indice_desag_OT == "rail_passenger") + share_OT_FinalEnergy_ref(k,indice_desag_OT == "rail_freight"))  .. // - coal
												- Qref(k,indice_OT) .* CI_OT_serv(indice_elec,indice_OT,k) .*share_OT_FinalEnergy_ref(k,indice_desag_OT == "rail_passenger") ./(share_OT_FinalEnergy_ref(k,indice_desag_OT == "rail_passenger") + share_OT_FinalEnergy_ref(k,indice_desag_OT == "rail_freight"))) .. // - elec
												./ EnergyServices_rail_pass(k);		
												
	alpharoad_freight_ref (k,indice_Et) 		= (sum(Qref(k,indice_OT) .* CI_OT_serv(energyIndexes,indice_OT,k) .*share_OT_FinalEnergy_ref(k,indice_desag_OT == "road_freight")) ..   // Total final energy of this mode according to the share of FE
												- Qref(k,indice_OT) .* CI_OT_serv(indice_oil,indice_OT,k)) .. // - oil
												./ EnergyServices_road_fret(k);
												
	alpharoad_passenger_ref (k,indice_Et) 	= (sum(Qref(k,indice_OT) .* CI_OT_serv(energyIndexes,indice_OT,k) .*share_OT_FinalEnergy_ref(k,indice_desag_OT == "road_passenger")) .. // Total final energy of this mode according to the share of FE
												- Qref(k,indice_OT) .* CI_OT_serv(indice_gaz,indice_OT,k)) .. // final energy gas for this mode
