...
// The state and household budget calibraton is done through the dividend rate.
// The equilibrium is made through the following two equations
// Households:
...
case "MAGE_MODEL"
    ...
    savings_rate_ssp2 = csvRead(DATA_SAVINGS +"savings_rate_ssp2.csv",';',[],[],[],'/\/\//'); 
    ptc=1-savings_rate_ssp2(:,1);
    div= ( (sum(DFref.*pArmDFref,'c')) ./ (1-IR)./ ptc +sum(DGref.*pArmDGref,'c')-sum(SALNET+TAXSAL,'c')-sum(QTAX+TAXCI+MTAXCI+TAXDF+TAXDG+TAXDI+XTAX+TTAX,'c'))./sum(EBEref,'c');
    transfersref=sum(QTAX+TAXCI+MTAXCI+TAXDF+TAXDG+TAXDI+XTAX+TTAX,'c')+sum(TAXSAL,'c')+IR.*sum(SALNET+(div*ones(1,sec)).*EBEref,'c')-sum(DGref.*pArmDGref,'c');
    Rdispref=sum(SALNET,'c')+div.*sum(EBEref,'c')+transfersref;
end
...
///////////////
// for the electricity sector, txCap comes the first execution of nexus.electricity with txCap=sum(Cap_elec_MW_dep+delta_Cap_elec_MW_1,'c')./sum(Cap_elec_MWref,'c')
...
txCap=csvRead( path_autocal_txCap+'/txCaptemp_'+string(nb_sectors)+'.csv',',');

if first_try_autocalib==%f
    txCap_elec=csvRead( path_autocal_txCap+'elec/txCaptemp_elec_'+string(nb_sectors)+'.csv',',');
...
if auto_calibration_txCap<>"None" & first_try_autocalib==%t
    ...
    pCap_elec_ref = pCap(:,indice_elec);
else
    pCap_elec_ref = csvRead(path_capital_elec+'pCap_elec_'+string(nb_sectors)+'.csv','|',[],[],[],'/\/\//');
    pCap(:,indice_elec) = pCap_elec_ref;
end
...
// Warning: Beta is multiplied by 1e6 for accuracy pruposes

// In the following loop i is a sector, j a good
// DI(k,j) is the demand of good j in order to build capital goods (sum over i) in region k
// NB: DI(k,j) = 0 for j=1:5 because goods j produced by energy sectors does not enter capital formation
// Beta(j,i,k) = quantity of good j entering the construction of one unit of capital for sector i in region k
// We assume colinearity between sectors: capital goods are identical (in terms of share of j there are built of) across the sector i requiring capital formation
// suppose Part_Compo_Cap(k,j) the vector of shares (the sum equal 1)
// then Beta(j,i,k) = Part_Compo_Cap(k,j) * Y(k,i) 
// where Y(k,i) is the agrgegate demand for capital (at price pK) of sector i
// pK(k,i) is the aggregate price of capital at construction for sector i in region k 
// DIref(k,j) is the demand of good j entering eavery capital formation for all sectors i (sum over i for region k)

// Beta(j,i,k) = quantity of good j entering one unit of capital formation of sector i in region k

// Computation methods for Beta:
// -------------------------
// BETA(j,i) results for a spread (towards sectors i demanding capital goods) of the aggregaete demand DIref(j) in capital good j
// at prorata of their respective "weights" of investment demand, in value, of each sector i in total investmentr of region k
// i.e 
// BETA(j,i) = DIref(j) * V_ref(i) / Vtot_ref
// where
// Vtot_ref = sum_j(pArmDIref(j).*DIref(j)) is the total value of investment in region k
// V_ref(i) = pK(i) * DKbrut(i) is the value of investment of sector i in region k
// with
// DKbrut(i) the variation of capital at the base year in sector i

// also
// Beta(j,i) = BETA(j,i) / DKbrut(i)

// GTAP data does not give this spread
// But we know DIref(k,j) at the base year, the aggregate demand over sector of good j entering capital formation
...
Inv_val_nonener = Inv_val - sum(pK(:,1:nbsecteurenergie).* deltaKbrut(:,1:nbsecteurenergie),'c');
pK(:,nbsecteurenergie+1:sec) = ( (Inv_val_nonener ./ sum( pref(:,nbsecteurenergie+1:sec) .* markup(:,nbsecteurenergie+1:sec) .* deltaKbrut(:,nbsecteurenergie+1:sec),'c')) * ones(1,sec-nbsecteurenergie)) .* pref(:,nbsecteurenergie+1:sec) .* markup(:,nbsecteurenergie+1:sec); 
...
partExpKmin= 1-NRBref./GRBref;
partExpK=ones(reg,1)*(max(partExpKmin)+0.05); // 0.01 is arbitrary, it is the minimum value of partExpK. The larger it is, the larger the initial values of partImpK will be. 
...
savings_rate_ssp2 = csvRead(DATA_SAVINGS +"savings_rate_ssp2.csv",';',[],[],[],'/\/\//'); 
//invest_rate_ssp2 = csvRead(DATA_SAVINGS +"invest_rate_ssp2.csv",';',[],[],[],'/\/\//');	
//savings_rate_ssp = csvRead(DATA_SAVINGS +"savings_rate_ssp"+ind_ssp_savings+".csv",';',[],[],[],'/\/\//'); 
//invest_rate_ssp = csvRead(DATA_SAVINGS +"invest_rate_ssp"+ind_ssp_savings+".csv",';',[],[],[],'/\/\//'); 
// From start_year_policy on, adopt savings & investments rates from ssp

savings_rate_ssp=savings_rate_ssp2.*[ones(reg,start_year_policy-base_year_simulation),ones(reg,1)*(1+(SavingsShift/(TimeHorizon-start_year_policy+base_year_simulation)*0.01*(0:TimeHorizon-(start_year_policy-base_year_simulation))))];
//invest_rate_ssp=invest_rate_ssp2; 
invest_rate_ssp = savings_rate_ssp;

saving_obj_exo_1 = savings_rate_ssp;
auto_invest_obj_exo_1 = invest_rate_ssp;

if isdef("ind_ssp_fin")
    if ind_ssp_fin==3
        cor_saving_bra = 80/100;
        saving_obj_exo_1(ind_bra,2050-base_year_simulation:$) = linspace(saving_obj_exo_1(ind_bra,2050-base_year_simulation),saving_obj_exo_1(ind_bra,2050-base_year_simulation)*cor_saving_bra, TimeHorizon+1-(2050-base_year_simulation)+1);
        auto_invest_obj_exo_1(ind_bra,2050-base_year_simulation:$) = linspace(auto_invest_obj_exo_1(ind_bra,2050-base_year_simulation),auto_invest_obj_exo_1(ind_bra,2050-base_year_simulation)*cor_saving_bra, TimeHorizon+1-(2050-base_year_simulation)+1);
    end
end

div_evolution = [ones(reg,1) (1-auto_invest_obj_exo_1(:,2:$))] ./ [ones(reg,1) (1-auto_invest_obj_exo_1(:,1:$-1))];
// In a variant (change_div_for_charge=%t, default %f) the dividende rate path has been adjusted (using batch/autocalibration_K.sh procedure)
// In order to correct for very high load rates (charge >> 0.85 in industries and services) in Africa
// This variante significantly increase Africa's GDP
div_evolution_sav = div_evolution;
if change_div_for_charge == %t
    for k=1:nb_regions
        filename = path_autocal_K+"change_div_"+string(k)+".csv";
        if isfile(filename)
            change_div = csvRead(filename);
            div_evolution(k,:) = div_evolution(k,:) .* change_div ./ [1, change_div(1:$-1)];
        end
    end
    // No change on the first years
    div_evolution(:,1:7) = div_evolution_sav(:,1:7);
end
...
K_shortage_index = zeros(reg,1);
sg_add('K_shortage_index');
partImpK_targ = partImpK;
sg_add('partImpK_targ');
// The higher alpha_FCC, the more international finance will be channeled to high-load regions.
alpha_FCC = ones(reg);
 
alpha_GDP= ones(reg);
sg_add('partImpK');
partImpK_weight=ones(reg);
partImpK_weight_prev=partImpK_weight;
sg_add('partImpK_weight');
tau_ImpK=10;
