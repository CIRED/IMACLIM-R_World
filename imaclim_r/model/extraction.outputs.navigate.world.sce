...
// but with exceptions
list_techind_temp=[indice_CSP,indice_GGS, indice_WNO, indice_BIGCCS, indice_BIGCC];
...
list_var_indexes = list(technoCoalWCCS, technoCoalWOCCS,technoGasWOCCS,technoElecNuke,technoElecHydro,technoPV);
...
list_var_indexes = list(technoCoalWCCS, technoCoalWOCCS,technoGasWOCCS,technoElecNuke);
for i=1:size(list_var_name,'c')
    indexes_var=list_var_indexes(i);
...
if ind_NLU > 0
    ...
    + outputs_temp(nbLines*[0:11] + line_BIGCCS,ind_time) ./ rho_elec_nexus(:,indice_BIGCCS) ..
    + outputs_temp(nbLines*[0:11] + line_BIGCC,ind_time) ./ rho_elec_nexus(:,indice_BIGCC);
