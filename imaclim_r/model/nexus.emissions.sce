...
if nb_sectors_industry==1
    ...
    CO2_intensity_industry = sum(Q(:,indus) .* sum(coef_Q_CO2_CI(:,indus,:) .* CI(:,indus,:),1), "c") ./ sum(Q(:,indus), "c");
end
...
elseif current_time_im==start_year_policy-base_year_simulation
    ...
    CO2_indus_process = CO2_indus_process_ref ./ CO2_intensity_industry_ref .* CO2_intensity_industry;
end
