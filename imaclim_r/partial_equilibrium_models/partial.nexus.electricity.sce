...
run_partial_eq_elec = %t; //TB: rustine pour mettre en USD2014 les LCOE des fossiles quand on lance le 
//nexus en 2014. Quand les coûts seront tous en USD2014, deviendra redondant
...
pArmCIref(indice_gaz,indice_elec,:) = p_gas_run(:,1)';
...
if  baseyear == 2002
CI(indice_gaz,indice_elec,:) =  [0.4171839, 0.1544484, 0.3478323, 0.4332054, 1.6360584, 0.0120240, 0.1976473, 0.0761202, 1.3640691, 0.7051759, 0.8160100, 0.6049082];
...
elseif baseyear == 2015
CI(indice_gaz,indice_elec,:) = [0.60927254, 0.42321913, 0.26938816, 0.79807711, 1.65894085, 0.049913, 0.13674186, 0.38373766, 1.43648618, 1.00276188, 1.04627473, 0.89358858]  ;
...
CIref = CI;
CI_prev = CI;
ldsav('Cum_Inv_MW_elec_ref_temp','calib'); //TB: ne trouve pas la source de Cum_Inv_MW_elec_ref_temp. Pour certaines technos, remplace Cum_Inv_MW_elec_ref si plus grand (l.1202 calib). Intervient dans le nexus si indice_calib_CINV_elec = 0
...
//TB:Taxe en USD2001(2014)/tCO2, intensité des émissions en tCO2/Mteo donc divise par 1e6 pour avoir une taxe en USD/teo
    ...
    pArmCI_no_taxCO2(indice_gaz,indice_elec,:) = p_gas_run(:,1+current_time_im);
