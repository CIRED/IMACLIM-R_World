...
folder <- paste0(SUMMARIZE,"/summarizeData/products/")
...
{
  temp1 <- dsAgg %>% dplyr::filter(var=="I", industry==industryHere)
  temp00 <- dsAgg %>% dplyr::filter(var=="VA", industry=="TOT" , country %in% as.vector(countries)) 
  ...
  {
    temp2 <- temp1 %>% dplyr::filter(country==countryHere) %>% 
    ...
    temp0 <- temp00 %>% dplyr::filter(country==countryHere) %>% select(-c(product,industry)) %>% spread(var,value)
    
    for(productHere in productsL) 
    {  
      temp3 <- temp2 %>% dplyr::filter(product==productHere) %>% spread(var,value)
      temp  <- left_join(temp0,temp3, by=c("country","year")) %>% mutate(I_VA=I/VA)
...
trends <- trends[,c("GFCF","NonICT","ICT","Con","RStruc","OCon","TraEq","OMach","Other","CT","IT","Soft")]
pvalues <- pvalues[,c("GFCF","NonICT","ICT","Con","RStruc","OCon","TraEq","OMach","Other","CT","IT","Soft")]
...
pvalues.cat <- pvalues.cat[,c("GFCF","NonICT","ICT","Con","RStruc","OCon","TraEq","OMach","Other","CT","IT","Soft")]
...
print(xtable(trends.final,caption=paste("Linear trend I/VA per country, 1970-2010 ", industryHere, sep="")),type="latex",size="\\scriptsize",file=paste(paste("Summary","SLR","I-VA","percountry",industryHere,sep="_"),".tex",sep=""))
} 
