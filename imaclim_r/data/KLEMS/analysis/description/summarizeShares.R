...
folder <- paste0(SUMMARIZE,"/summarizeShares/")
...
industryVector = c("AtB","C","D","E","F","GtH","I","J","70","71t74","LtQ")

for (countryHere in c("USA"))
{
  for (varHere in c("I"))
  {
    for (industryHere in c("TOT",industryVector){}
  }
  
  temp <- dsAgg %>% filter(country==countryHere, var==varHere, industry==industryHere) %>% spread(product,value)
  temp.shares <- temp %>% mutate_each(funs(./GFCF), -country, -var, -year, -industry, -GFCF) %>% select(-c(GFCF, NonICT, ICT))
                          gather(product, value, ) 
