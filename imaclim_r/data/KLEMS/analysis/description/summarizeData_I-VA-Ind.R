...
{
  temp1 <- dsAgg %>% dplyr::filter(var=="VA", industry=="TOT" , country %in% as.vector(countries), country==countryHere) %>% spread(var,value) %>% select(-c(product,industry))
  temp00 <- dsAgg %>% dplyr::filter(var=="I", country==countryHere)
  
  for (productHere in c("GFCF"))
  {
    temp0 <-temp00 %>% dplyr::filter(product==productHere)# %>% 
    ...
    {  
      temp2 <- temp0 %>% dplyr::filter(industry==industryHere) %>% spread(var,value)
      temp  <- left_join(temp1,temp2, by=c("country","year")) %>% mutate(I_VA=I/VA)
...
print(xtable(trends.final,caption=paste("Trend industries' investment share of VA per country, 1970 - 2010, all assets", sep="")),type="latex",size="\\scriptsize",file=paste(paste("Summary","SLR","industries-I_VA","percountry",productHere,sep="_"),".tex",sep=""))
