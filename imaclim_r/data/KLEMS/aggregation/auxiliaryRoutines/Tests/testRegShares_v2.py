...
with open('input/dicoRegion.csv', 'r') as f: #this file specifies how to aggregate the industries of KLEMS data to 12 sector IMACLIM format
    reader = csv.reader(f,delimiter='|',quoting=csv.QUOTE_NONE)
...
#for c in range(1):
#    for i in range(len(KLEMStoIMACLIMindustries)/2):
#    for i in range(1):

#        for k in range(len(KLEMStoIMACLIMindustries[i*2][1:])):
            
#            if KLEMStoIMACLIMindustries[i*2][k+1] in disaggInd:
                #print ("Yey")

#                RegionsKLEMStoIMACLIMindustries[c][1+2*i+1][k+1]=14 #[c][RegionsCapShares[]]
 #                      =RegionsCapShares[(c*6+c)+2*(1+disaggInd.index(KLEMStoIMACLIMindustries[i*2][k+1]))] #replace 6 by 2*len(disaggInd)
#                              [RegionsCapShares[(c*6+c)+2*(1+disaggInd.index(KLEMStoIMACLIMindustries[i*2][k+1]))].index(KLEMStoIMACLIMindustries[i*2][0])]
...
print "row ", (c*6+c)+2*(1+disaggInd.index(KLEMStoIMACLIMindustries[i*2][k+1]))

print "column of ", RegionsCapShares[(c*6+c)+2*(1+disaggInd.index(KLEMStoIMACLIMindustries[i*2][k+1]))-1]#.index(KLEMStoIMACLIMindustries[i*2][0])

print "looking for ", KLEMStoIMACLIMindustries[i*2][0]
...
print 'IMAsectorMatches', IMAsectorMatches

print "column ", RegionsCapShares[(c*6+c)+2*(1+disaggInd.index(KLEMStoIMACLIMindustries[i*2][k+1]))-1].index(IMAsectorMatches[0])
