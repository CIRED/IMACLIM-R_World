...
names(data_agreg) <- c("Region","Country","ISO","RE_or_NRE","Technology_type","Technology","Grid","Year","Installed_Cap")
...
#Set white spaces to NA and get numeric variable on Installed_Cap col
data_agreg$Installed_Cap <- gsub(" ","",data_agreg$Installed_Cap) %>% as.numeric()
...
#don't know why filter(data_agreg, !is.na(Installed_Cap)) does not work
data_agreg <- data_agreg[rowSums(is.na(data_agreg)) == 0,]

#Get the wanted dataset
data_agreg <- data_agreg %>% select(Region, Country, ISO, Technology, Installed_Cap,Year)
...
filter(data_agreg,IMACLIM_region == 0) %>% select(Installed_Cap) %>% sum()

#check if there is any big mistake (>100MW)
filter(data_agreg,IMACLIM_region == 0 & Installed_Cap > 100) %>% group_by(Country) %>% summarize()
...
if(year == annee){
  Cap<- aggregate(data_agreg$Installed_Cap, by = list(data_agreg$Technology,data_agreg$IMACLIM_region), FUN = sum)
  names(Cap) <- c("Technology","Region","Installed_Cap")}
