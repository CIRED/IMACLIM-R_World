...
// Licence: AGPL-3.0
// Authors:
//     Aurélie Méjean, Florian Leblanc
...
sigma_income_country = sqrt(2)*cdfnor("X",zeros(nb_countries,nb_steps),ones(nb_countries,nb_steps),(income_gini_country(:,3:$) + 1)/2,1-(income_gini_country(:,3:$) + 1)/2);
...
matrix_quantiles = zeros(nb_countries,nb_quantiles_Lorenz,nb_steps);
gdp_quantiles = zeros(nb_countries,nb_quantiles_Lorenz,nb_steps);
pop_quantiles = zeros(nb_countries,nb_quantiles_Lorenz,nb_steps);

quantiles_cum_reg =zeros(reg,nb_quantiles,nb_steps,2); //"2" dimension is 1 for cumulative population share, 2 for cumulative gdp share
...
income_gini_reg = zeros(reg,nb_steps);
cons_gini_reg = zeros(reg,nb_steps);
income_gini_reg_check = zeros(reg,nb_steps);

for i_step = 1:nb_steps
...
for i=1:nb_quantiles
    for j=1:2
        csvWrite( matrix(inc_quantiles_reg(:,i,:,j),reg,nb_steps), output_dir + 'inc_quantiles_reg_quantile'+string(i)+'_popORgdpShare'+string(j)+'.csv', '|', [], [], header);
        csvWrite( matrix(cons_quantiles_reg(:,i,:,j),reg,nb_steps), output_dir + 'cons_quantiles_reg_quantile'+string(i)+'_popORgdpShare'+string(j)+'.csv', '|', [], [], header);
    end
end
