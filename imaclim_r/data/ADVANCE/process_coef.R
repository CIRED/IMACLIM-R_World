...
if (var_reg == "Res_peak"){
fitted_mod <- RLDC %>% group_by(Region) %>% do(model = lm(I(get(var_reg)-1) ~ 0 + Wind_share + PV_share 
                                                          + PV_share*Wind_share + Wind_share_sq + PV_share_sq 
                                                          + Wind_share_sq*PV_share + PV_share_sq*Wind_share
                                                          + Wind_share_cb + PV_share_cb,
                                                          data = .))
...
if (var_reg == "Res_peak"){
  coef_export  <- tibble(rep(1,nrow(coef_export)),coef_export)
  names(coef_export) <- c("Intercept",names(coefAFR))
}

if (var_reg == "Curtailment") {
...
#kbl(coef_export, "latex",caption = "Coefficients of regression for the twelve IMACLIM-R regions - Curtailement (in % of demand)", booktabs = T) %>% kable_styling(latex_options = c("striped", "scale_down"))
