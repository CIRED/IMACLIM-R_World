...
// Licence: AGPL-3.0
// Authors:
//     Florian Leblanc, Adrien Vogt-Schilb, Julie Rozenberg
//     (CIRED - CNRS/AgroParisTech/ENPC/EHESS/CIRAD)
// =============================================

function plotreg(mat,regs,pos)
    // plot of regional series in one line
    // plotreg(L, [regs, [pos]])
    // L: imaclim matrix reg*something
    // regs: optionnal, vector of regional indicesd to take into account. by default 1:reg
    // pos: optional (3 by default). Where to be the legend box
    // see 'legend' help to position the legend. pos=5 for example.
...
function nice_plot ( title_str,xlabs_str, ylabs_str, is2030)
    ...
    //Add a bit title, an x and an y label
    // All argument are oiptional
    //examples
    // nice_plot  (without argiment) : write year from 2000 oin the x-axis
    //nice_plot ('hy') : x axes and plot title 
    //nice_plot( 'world price', 'dollars per baril') :  nice x axe, cool y axe, plot title
...
function plot_bin(var,unit,binaryString,timeSpan)
    //Plot the variable whose name is  m_var" in 2 colours, function of the value of binaryString

    output = evstr('m_' + var)/unit;
    ...
    binaryTest = evstr(binaryString)
        
    clf
    if sum(~binaryTest) //avoid mistake because of empty matrix
...
function plot_means(var,unit, varargin)
    //spaguettis grey plot with thick colour lines
    //var : string. a matrix m_ should exists with the same name
    //unit : number, unit of m_var
    //varargin : list of strings containing the name of tests
...
function plot_pastout(var,unit,binaryString,varargin)
    //same as plot_means but by keeping a subpart of the matrix m_series
    // hc plot_means
    ...
    //if there is means, we set it in grey
    if length(varargin)>0
        curve_style('gray','line',2)
    end
