...
function [y1] = DFinduite(Consoloc,Tautomobileloc) ;
    //Compute the final energy demand of households, function of their consumption
    // for now housing is not included
    ...
    y1(:,indice_gas)=alphaGazm2.*stockbatiment;
...
/////////////////// Transport time function for each transport mode ////////////////////////////////////////////////////////////////
...
// markup for all sectors
// we distinguish later sectors one by one when we study production function
...
function [y] = Sector_budget(ploc,wloc,Qloc);
    ...
    //
    y=(A_CI.*costs_CI+A.*wloc.*l.*(1+sigma).*(energ_sec+FCCtemp.*non_energ_sec)+FCCmarkup_oil.*markup.*ploc.*((energ_sec+non_energ_sec))).*(1+qtax)-ploc;
    //
...
function [gap] = emissions_gap(Qloc,DFloc,DIloc,Imptemp,Exptemp,TAXVALloc)
    //constraint: emissions target an fsolve will find the zeros 
    //Calculates emissions from the variables Qloc DFloc  and DIloc    
    //and compares them to the CO2_obj_MKTparam parameter
    //the areEmiConstr parameter disables the constraint on emissions
...
function E_reg_use = emissions_usage(CI,Qloc,DFloc,DIloc)
    //Emissions by region and usage. See nb_use in indexes.sce
...
function Ener_reg_use = energie_usage(CI,Q,DF,DI,DG)
    //Energy consumption of fossil fuel by region and usage. See nb_use in indexes.sce
...
function [y,Qloc,DFloc,DIloc,Imptemp,Exptemp] = economy(x);
    ...
    //if fsolve call this function, it will keep x
    //the extra output can eventually be used by otehr functions
    ...
    FCCtemp=aRD+bRD.*tanh(cRD.*(A.*Qloc./Cap-1)); 
    FCCmarkup=ones(reg,sec);
    FCCmarkup=((markup_lim_oil-markupref)/(1-0.8).*(A.*Qloc./Cap-0.8*ones(reg,sec))+markupref)./markup; 
    ...
    //particular case for oil: we prevent region with a high load rate to consume its domestic oil production
    ...
    //for oil market: we prevent region with a high load rate to export
    ...
    DIloc=DIinfra+DIprod.*(((NRBtemp-sum(DIinfra.*pArmDI,'c'))./sum(DIprod.*pArmDI,'c'))*ones(1,sec));

    /////////// intermediate computation of exports & imports
    QCdomtemp=ones(reg,sec);
    QCdomtemp=[(A_CI(1,:).*Qloc(1,:))*((CI(:,:,1).*partDomCI(:,:,1))');
    (A_CI(2,:).*Qloc(2,:))*((CI(:,:,2).*partDomCI(:,:,2))');
    (A_CI(3,:).*Qloc(3,:))*((CI(:,:,3).*partDomCI(:,:,3))');
    (A_CI(4,:).*Qloc(4,:))*((CI(:,:,4).*partDomCI(:,:,4))');
    (A_CI(5,:).*Qloc(5,:))*((CI(:,:,5).*partDomCI(:,:,5))');
    (A_CI(6,:).*Qloc(6,:))*((CI(:,:,6).*partDomCI(:,:,6))');
    (A_CI(7,:).*Qloc(7,:))*((CI(:,:,7).*partDomCI(:,:,7))');
    (A_CI(8,:).*Qloc(8,:))*((CI(:,:,8).*partDomCI(:,:,8))');
    (A_CI(9,:).*Qloc(9,:))*((CI(:,:,9).*partDomCI(:,:,9))');
    (A_CI(10,:).*Qloc(10,:))*((CI(:,:,10).*partDomCI(:,:,10))');
    (A_CI(11,:).*Qloc(11,:))*((CI(:,:,11).*partDomCI(:,:,11))');
    (A_CI(12,:).*Qloc(12,:))*((CI(:,:,12).*partDomCI(:,:,12))')]+DFloc.*partDomDF+DG.*partDomDG+DIloc.*partDomDI;

    Imptemp=ones(reg,sec);
    Imptemp=[(A_CI(1,:).*Qloc(1,:))*((CI(:,:,1).*(partImpCI(:,:,1)))');
    (A_CI(2,:).*Qloc(2,:))*((CI(:,:,2).*(partImpCI(:,:,2)))');
    (A_CI(3,:).*Qloc(3,:))*((CI(:,:,3).*(partImpCI(:,:,3)))');
    (A_CI(4,:).*Qloc(4,:))*((CI(:,:,4).*(partImpCI(:,:,4)))');
    (A_CI(5,:).*Qloc(5,:))*((CI(:,:,5).*(partImpCI(:,:,5)))');
    (A_CI(6,:).*Qloc(6,:))*((CI(:,:,6).*(partImpCI(:,:,6)))');
    (A_CI(7,:).*Qloc(7,:))*((CI(:,:,7).*(partImpCI(:,:,7)))');
    (A_CI(8,:).*Qloc(8,:))*((CI(:,:,8).*(partImpCI(:,:,8)))');
    (A_CI(9,:).*Qloc(9,:))*((CI(:,:,9).*(partImpCI(:,:,9)))');
    (A_CI(10,:).*Qloc(10,:))*((CI(:,:,10).*(partImpCI(:,:,10)))');
    (A_CI(11,:).*Qloc(11,:))*((CI(:,:,11).*(partImpCI(:,:,11)))');
    (A_CI(12,:).*Qloc(12,:))*((CI(:,:,12).*(partImpCI(:,:,12)))')]+DFloc.*(partImpDF)+DG.*(partImpDG)+DIloc.*(partImpDI);
    ...
    taxCItemp=[
    sum((ploc(1,:)'*ones(1,sec)).*taxCIdom(:,:,1).*partDomCI(:,:,1).*CI(:,:,1).*(ones(sec,1)*(Qloc(1,:).*A_CI(1,:))))+sum(((wploc.*(1+mtax(1,:))+nit(1,:).*wpTIaggloc)'*ones(1,sec)).*taxCIimp(:,:,1).*(partImpCI(:,:,1)).*CI(:,:,1).*(ones(sec,1)*(Qloc(1,:).*A_CI(1,:))));
    sum((ploc(2,:)'*ones(1,sec)).*taxCIdom(:,:,2).*partDomCI(:,:,2).*CI(:,:,2).*(ones(sec,1)*(Qloc(2,:).*A_CI(2,:))))+sum(((wploc.*(1+mtax(2,:))+nit(2,:).*wpTIaggloc)'*ones(1,sec)).*taxCIimp(:,:,2).*(partImpCI(:,:,2)).*CI(:,:,2).*(ones(sec,1)*(Qloc(2,:).*A_CI(2,:))));
    sum((ploc(3,:)'*ones(1,sec)).*taxCIdom(:,:,3).*partDomCI(:,:,3).*CI(:,:,3).*(ones(sec,1)*(Qloc(3,:).*A_CI(3,:))))+sum(((wploc.*(1+mtax(3,:))+nit(3,:).*wpTIaggloc)'*ones(1,sec)).*taxCIimp(:,:,3).*(partImpCI(:,:,3)).*CI(:,:,3).*(ones(sec,1)*(Qloc(3,:).*A_CI(3,:))));
    sum((ploc(4,:)'*ones(1,sec)).*taxCIdom(:,:,4).*partDomCI(:,:,4).*CI(:,:,4).*(ones(sec,1)*(Qloc(4,:).*A_CI(4,:))))+sum(((wploc.*(1+mtax(4,:))+nit(4,:).*wpTIaggloc)'*ones(1,sec)).*taxCIimp(:,:,4).*(partImpCI(:,:,4)).*CI(:,:,4).*(ones(sec,1)*(Qloc(4,:).*A_CI(4,:))));
    sum((ploc(5,:)'*ones(1,sec)).*taxCIdom(:,:,5).*partDomCI(:,:,5).*CI(:,:,5).*(ones(sec,1)*(Qloc(5,:).*A_CI(5,:))))+sum(((wploc.*(1+mtax(5,:))+nit(5,:).*wpTIaggloc)'*ones(1,sec)).*taxCIimp(:,:,5).*(partImpCI(:,:,5)).*CI(:,:,5).*(ones(sec,1)*(Qloc(5,:).*A_CI(5,:))));
    sum((ploc(6,:)'*ones(1,sec)).*taxCIdom(:,:,6).*partDomCI(:,:,6).*CI(:,:,6).*(ones(sec,1)*(Qloc(6,:).*A_CI(6,:))))+sum(((wploc.*(1+mtax(6,:))+nit(6,:).*wpTIaggloc)'*ones(1,sec)).*taxCIimp(:,:,6).*(partImpCI(:,:,6)).*CI(:,:,6).*(ones(sec,1)*(Qloc(6,:).*A_CI(6,:))));
    sum((ploc(7,:)'*ones(1,sec)).*taxCIdom(:,:,7).*partDomCI(:,:,7).*CI(:,:,7).*(ones(sec,1)*(Qloc(7,:).*A_CI(7,:))))+sum(((wploc.*(1+mtax(7,:))+nit(7,:).*wpTIaggloc)'*ones(1,sec)).*taxCIimp(:,:,7).*(partImpCI(:,:,7)).*CI(:,:,7).*(ones(sec,1)*(Qloc(7,:).*A_CI(7,:))));
    sum((ploc(8,:)'*ones(1,sec)).*taxCIdom(:,:,8).*partDomCI(:,:,8).*CI(:,:,8).*(ones(sec,1)*(Qloc(8,:).*A_CI(8,:))))+sum(((wploc.*(1+mtax(8,:))+nit(8,:).*wpTIaggloc)'*ones(1,sec)).*taxCIimp(:,:,8).*(partImpCI(:,:,8)).*CI(:,:,8).*(ones(sec,1)*(Qloc(8,:).*A_CI(8,:))));
    sum((ploc(9,:)'*ones(1,sec)).*taxCIdom(:,:,9).*partDomCI(:,:,9).*CI(:,:,9).*(ones(sec,1)*(Qloc(9,:).*A_CI(9,:))))+sum(((wploc.*(1+mtax(9,:))+nit(9,:).*wpTIaggloc)'*ones(1,sec)).*taxCIimp(:,:,9).*(partImpCI(:,:,9)).*CI(:,:,9).*(ones(sec,1)*(Qloc(9,:).*A_CI(9,:))));
    sum((ploc(10,:)'*ones(1,sec)).*taxCIdom(:,:,10).*partDomCI(:,:,10).*CI(:,:,10).*(ones(sec,1)*(Qloc(10,:).*A_CI(10,:))))+sum(((wploc.*(1+mtax(10,:))+nit(10,:).*wpTIaggloc)'*ones(1,sec)).*taxCIimp(:,:,10).*(partImpCI(:,:,10)).*CI(:,:,10).*(ones(sec,1)*(Qloc(10,:).*A_CI(10,:))));
    sum((ploc(11,:)'*ones(1,sec)).*taxCIdom(:,:,11).*partDomCI(:,:,11).*CI(:,:,11).*(ones(sec,1)*(Qloc(11,:).*A_CI(11,:))))+sum(((wploc.*(1+mtax(11,:))+nit(11,:).*wpTIaggloc)'*ones(1,sec)).*taxCIimp(:,:,11).*(partImpCI(:,:,11)).*CI(:,:,11).*(ones(sec,1)*(Qloc(11,:).*A_CI(11,:))));
    sum((ploc(12,:)'*ones(1,sec)).*taxCIdom(:,:,12).*partDomCI(:,:,12).*CI(:,:,12).*(ones(sec,1)*(Qloc(12,:).*A_CI(12,:))))+sum(((wploc.*(1+mtax(12,:))+nit(12,:).*wpTIaggloc)'*ones(1,sec)).*taxCIimp(:,:,12).*(partImpCI(:,:,12)).*CI(:,:,12).*(ones(sec,1)*(Qloc(12,:).*A_CI(12,:))))];

    TAXCO2temp_dom=[(A_CI(1,:).*Qloc(1,:))*((CI(:,:,1).*partDomCI(:,:,1).*taxCO2_CI(:,:,1).*coef_Q_CO2_CI(:,:,1).*(numloc(1,1)*ones(sec,sec)))');
    (A_CI(2,:).*Qloc(2,:))*((CI(:,:,2).*partDomCI(:,:,2).*taxCO2_CI(:,:,2).*coef_Q_CO2_CI(:,:,2).*(numloc(2,1)*ones(sec,sec)))');
    (A_CI(3,:).*Qloc(3,:))*((CI(:,:,3).*partDomCI(:,:,3).*taxCO2_CI(:,:,3).*coef_Q_CO2_CI(:,:,3).*(numloc(3,1)*ones(sec,sec)))');
    (A_CI(4,:).*Qloc(4,:))*((CI(:,:,4).*partDomCI(:,:,4).*taxCO2_CI(:,:,4).*coef_Q_CO2_CI(:,:,4).*(numloc(4,1)*ones(sec,sec)))');
    (A_CI(5,:).*Qloc(5,:))*((CI(:,:,5).*partDomCI(:,:,5).*taxCO2_CI(:,:,5).*coef_Q_CO2_CI(:,:,5).*(numloc(5,1)*ones(sec,sec)))');
    (A_CI(6,:).*Qloc(6,:))*((CI(:,:,6).*partDomCI(:,:,6).*taxCO2_CI(:,:,6).*coef_Q_CO2_CI(:,:,6).*(numloc(6,1)*ones(sec,sec)))');
    (A_CI(7,:).*Qloc(7,:))*((CI(:,:,7).*partDomCI(:,:,7).*taxCO2_CI(:,:,7).*coef_Q_CO2_CI(:,:,7).*(numloc(7,1)*ones(sec,sec)))');
    (A_CI(8,:).*Qloc(8,:))*((CI(:,:,8).*partDomCI(:,:,8).*taxCO2_CI(:,:,8).*coef_Q_CO2_CI(:,:,8).*(numloc(8,1)*ones(sec,sec)))');
    (A_CI(9,:).*Qloc(9,:))*((CI(:,:,9).*partDomCI(:,:,9).*taxCO2_CI(:,:,9).*coef_Q_CO2_CI(:,:,9).*(numloc(9,1)*ones(sec,sec)))');
    (A_CI(10,:).*Qloc(10,:))*((CI(:,:,10).*partDomCI(:,:,10).*taxCO2_CI(:,:,10).*coef_Q_CO2_CI(:,:,10).*(numloc(10,1)*ones(sec,sec)))');
    (A_CI(11,:).*Qloc(11,:))*((CI(:,:,11).*partDomCI(:,:,11).*taxCO2_CI(:,:,11).*coef_Q_CO2_CI(:,:,11).*(numloc(11,1)*ones(sec,sec)))');
    (A_CI(12,:).*Qloc(12,:))*((CI(:,:,12).*partDomCI(:,:,12).*taxCO2_CI(:,:,12).*coef_Q_CO2_CI(:,:,12).*(numloc(12,1)*ones(sec,sec)))')]+DFloc.*partDomDF.*taxCO2_DF.*coef_Q_CO2_DF.*numloc+DG.*partDomDG.*taxCO2_DG.*coef_Q_CO2_DG.*numloc+DIloc.*partDomDI.*taxCO2_DI.*coef_Q_CO2_DI.*numloc;

    TAXCO2temp_imp=[(A_CI(1,:).*Qloc(1,:))*((CI(:,:,1).*partImpCI(:,:,1).*taxCO2_CI(:,:,1).*coef_Q_CO2_CI(:,:,1).*(numloc(1,1)*ones(sec,sec)))');
    (A_CI(2,:).*Qloc(2,:))*((CI(:,:,2).*partImpCI(:,:,2).*taxCO2_CI(:,:,2).*coef_Q_CO2_CI(:,:,2).*(numloc(2,1)*ones(sec,sec)))');
    (A_CI(3,:).*Qloc(3,:))*((CI(:,:,3).*partImpCI(:,:,3).*taxCO2_CI(:,:,3).*coef_Q_CO2_CI(:,:,3).*(numloc(3,1)*ones(sec,sec)))');
    (A_CI(4,:).*Qloc(4,:))*((CI(:,:,4).*partImpCI(:,:,4).*taxCO2_CI(:,:,4).*coef_Q_CO2_CI(:,:,4).*(numloc(4,1)*ones(sec,sec)))');
    (A_CI(5,:).*Qloc(5,:))*((CI(:,:,5).*partImpCI(:,:,5).*taxCO2_CI(:,:,5).*coef_Q_CO2_CI(:,:,5).*(numloc(5,1)*ones(sec,sec)))');
    (A_CI(6,:).*Qloc(6,:))*((CI(:,:,6).*partImpCI(:,:,6).*taxCO2_CI(:,:,6).*coef_Q_CO2_CI(:,:,6).*(numloc(6,1)*ones(sec,sec)))');
    (A_CI(7,:).*Qloc(7,:))*((CI(:,:,7).*partImpCI(:,:,7).*taxCO2_CI(:,:,7).*coef_Q_CO2_CI(:,:,7).*(numloc(7,1)*ones(sec,sec)))');
    (A_CI(8,:).*Qloc(8,:))*((CI(:,:,8).*partImpCI(:,:,8).*taxCO2_CI(:,:,8).*coef_Q_CO2_CI(:,:,8).*(numloc(8,1)*ones(sec,sec)))');
    (A_CI(9,:).*Qloc(9,:))*((CI(:,:,9).*partImpCI(:,:,9).*taxCO2_CI(:,:,9).*coef_Q_CO2_CI(:,:,9).*(numloc(9,1)*ones(sec,sec)))');
    (A_CI(10,:).*Qloc(10,:))*((CI(:,:,10).*partImpCI(:,:,10).*taxCO2_CI(:,:,10).*coef_Q_CO2_CI(:,:,10).*(numloc(10,1)*ones(sec,sec)))');
    (A_CI(11,:).*Qloc(11,:))*((CI(:,:,11).*partImpCI(:,:,11).*taxCO2_CI(:,:,11).*coef_Q_CO2_CI(:,:,11).*(numloc(11,1)*ones(sec,sec)))');
    (A_CI(12,:).*Qloc(12,:))*((CI(:,:,12).*partImpCI(:,:,12).*taxCO2_CI(:,:,12).*coef_Q_CO2_CI(:,:,12).*(numloc(12,1)*ones(sec,sec)))')]+DFloc.*partImpDF.*taxCO2_DF.*coef_Q_CO2_DF.*numloc+DG.*partImpDG.*taxCO2_DG.*coef_Q_CO2_DG.*numloc+DIloc.*partImpDI.*taxCO2_DI.*coef_Q_CO2_DI.*numloc;
...
function y=economyX(x)
    //constraint in the economy function, plus additional constraint on carbon market
...
function [Conso , Tautomobile, lambda , mu , p , w , Q , Rdisp , TNM , wpEner ]=expand_equilibrium(x)
    //Spread the X vector given to the function "economy" (state variables of the equilibrium) into the appropriate variables
    //Case without carbon constraint
    ...
    //physical variables and consumption / basic-need variables are non negative
    for k=1:reg,
...
function [taxCO2_DF, taxCO2_DG, taxCO2_DI, taxCO2_CI, taxMKT]=expand_tax(x,taxCO2_DF, taxCO2_DG, taxCO2_DI, taxCO2_CI, taxMKTin, varargin)
    ...
    //case when teh carbon tax is computed in the static
...
function [ taxCO2_DF , taxCO2_DG , taxCO2_DI , taxCO2_CI , taxMKT ] = dispatchTax( taxCO2_DF , taxCO2_DG , taxCO2_DI , taxCO2_CI , taxMKT, varargin )
    ...
    for k=1:reg
        ...
        // we check in which market each sector are
        for i1=1:sec
            taxCO2_CI(:,i1,k)= weight_regional_tax_expand(k) .* taxMKT(whichMKT_reg_use(k,i1));
        end

        // ... and in which market we account for DI, DF, DG
    ...
    taxCO2_CI(1:nbsecteurenergie,[indice_coal indice_gas indice_oil],1:reg)=0;
...
/////////////////////fonction to spread the investment in the electric sector
// we go from a minimisatin problem under inequality constraint to a minimisation problem penalised under an equality constraint with positive variables
...
//fonction to compute Hubbert curves on the degrowth phase
...
function [y] =  calib_markup_gaz()

    costs_CI=zeros(reg,sec);
    ploc=p;
    ploc(:,indice_gas)=pref(:,indice_gas).*taux_gaz_prix*p(1,indice_composite);

    pArmCItemp=pArmCI;

    pArmCItemp(1:nbsecteurenergie,indice_gas,:)=((ploc(:,1:nbsecteurenergie)').*(1+matrix(taxCIdom(1:nbsecteurenergie,indice_gas,:),nbsecteurenergie,reg))).*(1-matrix(partImpCI(1:nbsecteurenergie,indice_gas,:),nbsecteurenergie,reg))+(((ones(12,1)*wpEner.*(1+mtax(:,1:nbsecteurenergie))+nit(:,1:nbsecteurenergie)*wpTIagg)').*(1+matrix(taxCIimp(1:nbsecteurenergie,indice_gas,:),nbsecteurenergie,reg))).*matrix(partImpCI(1:nbsecteurenergie,indice_gas,:),nbsecteurenergie,reg)+(matrix(taxCO2_CI(1:nbsecteurenergie,indice_gas,:),nbsecteurenergie,reg).*matrix(coef_Q_CO2_CI(1:nbsecteurenergie,indice_gas,:),nbsecteurenergie,reg)).*(num(:,1:nbsecteurenergie)');

    costs_CI(:,indice_gas)=sum(pArmCItemp(:,indice_gas,:).*CI(:,indice_gas,:),"r");
    dispoRess = Ress_gaz/sum(Ress_gaz);
    FCC_ant = FCC(:,gaz) .* (1 + (dispoRess < 0.01) .* (1 ./ 0.01) .* (dispoRess - 0.01));
    y = ( ploc(:,indice_gas) ./ (1+qtax(:,indice_gas)) - A(:,indice_gas).*(A_CI(:,indice_gas).*costs_CI(:,indice_gas)+w(:,indice_gas).*l(:,indice_gas).*(1+sigma(:,indice_gas)).*(energ_sec(:,indice_gas)+FCC_ant.*non_energ_sec(:,indice_gas))) ) ./ ploc(:,indice_gas);

endfunction

function [y] =  calib_markup_coal()
    ...
    y = ( ploc(:,indice_coal) ./ (1+qtax(:,indice_coal)) - A(:,indice_coal).*(A_CI(:,indice_coal).*costs_CI(:,indice_coal)+w(:,indice_coal).*l(:,indice_coal).*(1+sigma(:,indice_coal)).*(energ_sec(:,indice_coal)+FCC_ant.*non_energ_sec(:,indice_coal)))) ./ ploc(:,indice_coal);

endfunction

function [y] =  calib_markup_elec()
    ...
    y1 = (ploc ./ (1+qtaxref) - A.*(A_CI.*costs_CI+w.*l.*(1+sigma).*(energ_sec+FCC.*non_energ_sec)) + repmat(biomassTaxBack,1,sec) ) ./ ploc;
    y=y1(:,indice_elec);
...
function [y] =  calib_UR_oil_MO(xloc)
    ...
    //
    y1=((A_CI.*costs_CI+A.*w.*l.*(1+sigma).*(energ_sec+FCCtemp.*non_energ_sec))+FCCmarkup_oil.*markup.*ploc.*((energ_sec+non_energ_sec))).*(1+qtax)-ploc;
    y=y1(9,indice_oil);
...
function [y] =  calib_markup_oil(xloc)
    ...
    y=((A_CI(:,indice_oil).*costs_CI(:,indice_oil)+A(:,indice_oil).*w(:,indice_oil).*l(:,indice_oil).*(1+sigma(:,indice_oil)).*(energ_sec(:,indice_oil)+FCC_ant.*non_energ_sec(:,indice_oil)))+markup_temp.*ploc(:,indice_oil).*((energ_sec(:,indice_oil)+non_energ_sec(:,indice_oil)))).*(1+qtax(:,indice_oil))-ploc(:,indice_oil);
...
function [y] = K_cal_logit_ind(Kloc)
    p_ener_industrie=[pArmCI(indice_coal,indice_industries(sec_indus),pays);pArmCI(indice_Et,indice_industries(sec_indus),pays);pArmCI(indice_gas,indice_industries(sec_indus),pays);pArmCI(indice_elec,indice_industries(sec_indus),pays)];
    ...
    p_ener_industrie_CO2=[pArmCI_CO2(indice_coal,indice_industries(sec_indus),pays);pArmCI_CO2(indice_Et,indice_industries(sec_indus),pays);pArmCI_CO2(indice_gas,indice_industries(sec_indus),pays);pArmCI_CO2(indice_elec,indice_industries(sec_indus),pays)];
...
function [y] = K_cal_logit_agr(Kloc)
    p_ener_agriculture=[pArmCI(indice_coal,indice_agriculture,pays);pArmCI(indice_Et,indice_agriculture,pays);pArmCI(indice_gas,indice_agriculture,pays);pArmCI(indice_elec,indice_agriculture,pays)];
    ...
    p_ener_agriculture_CO2=[pArmCI_CO2(indice_coal,indice_agriculture,pays);pArmCI_CO2(indice_Et,indice_agriculture,pays);pArmCI_CO2(indice_gas,indice_agriculture,pays);pArmCI_CO2(indice_elec,indice_agriculture,pays)];
...
function [y] = K_cal_logit_ser(Kloc)
    p_ener_composite=[pArmCI(indice_coal,indice_composite,pays);pArmCI(indice_Et,indice_composite,pays);pArmCI(indice_gas,indice_composite,pays);pArmCI(indice_elec,indice_composite,pays)];
    ...
    p_ener_composite_CO2=[pArmCI_CO2(indice_coal,indice_composite,pays);pArmCI_CO2(indice_Et,indice_composite,pays);pArmCI_CO2(indice_gas,indice_composite,pays);pArmCI_CO2(indice_elec,indice_composite,pays)];
...
function [y] = IC_calibration_new(MS_arg,LCC,val_nu)
    ///MS: calibration market share sum(MS)=1, MS(k)<>0, vector of size n in column
    ///LCC: corresponding usage costs, vector of size n in column
    ///index_ref: number of the technology to be used for the calculation, integer
    /// -val_nu: power of the logit, real number
    //////processing of MS to remove zero values
...
function [y] = IC_calibration_newPb(MS_arg,LCC,val_nu)
    ...
    ///LCC: corresponding usage costs, vector of size n in column
    ///index_ref: number of the technology to be used for the calculation, integer
    /// -val_nu: logit power, real number 
    //////processing of MS to remove zero values
    sizeIC=size(MS_arg,'r');
    nu=-val_nu*ones(sizeIC,1);
    sum_temp=sum((LCC).^nu);
...
function [y] = IC_calibration_newGPb(MS_arg,LCC,val_nu)
    ...
    /// -val_nu: logit power, real number 
    //////processing of MS to remove zero values
    //modification of target shares so as not to stray too far from the situation being calibrated
    sizeIC=size(MS_arg,'r');
    nu=-val_nu*ones(sizeIC,1);
    sum_temp=sum((LCC).^nu);
    MS_ref=(LCC(1:sizeIC)).^nu(1:sizeIC)/sum_temp;
