...
// Licence: AGPL-3.0
// Authors:
//     Adrien Vogt-Schilb, Ruben Bibas, Céline Guivarch, Renaud Crassous, Henri Waisman, Olivier Sassi
...
//  Meant to be called by preamble
//
//////////////////////////////////////////////////////////////////
...
function y = trues( varargin )
    ...
    //special case when the input is a matrix (the output is then a boolean matrix with the same size)
    if argn(2)==1
        y = ones (varargin(1)) == 1
        return
    end

    //begining of the string to exec
    strexec ="y = ones("
  
    for i = 1:argn(2)
        //special case of a matrix dimenssion equal to 0
        if varargin(i)==0
            y=[]
            return
        end
        ...
        //add the next argument of the function
        strexec = strexec + varargin(i) +","
    end
   
    //delate the last comma and add the equality condition
    strexec = part (strexec,1:length(strexec)-1)+")==1";
    execstr(strexec) //finale execution of the string
...
function say( varargin )
    ...
    //case we give a list to tune function say
    ...
    if strsubst(thedisp," ","")~="", printf(thedisp+"\n"); end
endfunction

function out=titre_convul(regs,split)
    //from a list of region and a list of variables, gives back 2 columns that can be used as a title
...
function run_date=mydate(nombre)
    ...
    run_date = tempdate(:,1) +"_"+tmp2 +tempdate(:,2) +"_"+ tmp6 + tempdate(:,6) +"_"+tmp7+ tempdate(:,7)+"h"+ tmp8 + tempdate(:,8)+"min"+tmp9+tempdate(:,9)+"s";
endfunction 
...
// //build a string of type "arg1, arg2, arg3"
// //useful to encapsulate functions that uses varargin
